...
'@
        ...
        function script:Set-DefenseClawManagedServices {
            ...
            $script:HarnessState.service_start_modes[$GatewayServiceName] =
                $mode
            # The Broker leg drops out in the no-Broker restore lane (#850);
            # the sensor helper is Broker-independent and always registered,
            # so the harness stamps it unconditionally.
