...
# block is required for endpoint inventory to flow. home_dirs below is
# refreshed by com.cisco.secureclient.defenseclaw.hook-enumerator on every
# tick from the same eligible-user pass that renders
# hook-guardian/targets.yaml — do not hand-edit; changes will be overwritten.
# Without this list a launchd/root-launched daemon walks /var/root only and
# misses every user's per-user data (editor extensions, MCP configs, shell
# history).
ai_discovery:
  enabled: true
  home_dirs:
