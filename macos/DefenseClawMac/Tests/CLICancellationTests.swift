...
// Minimal standalone-test dependency for CLIRunner. The app target supplies
// the production installation identity and read-only policy.
struct InstallationContext: Equatable {
    struct AccessMode: Equatable {
        var reason: String?
    }

    var permitsMutation = true
    var runtimeCLIURL = URL(fileURLWithPath: "/usr/bin/false")
    var protectedSubprocessEnvironment: [String: String] = [:]
    var accessMode = AccessMode(reason: nil)

    static func resolve() -> InstallationContext {
        InstallationContext()
    }
}
