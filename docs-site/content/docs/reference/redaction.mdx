...
There is no v7 compatibility built-in. Upgrading a redacting v7 installation
writes an ordinary custom profile named `v7-compatible` into
`observability.redaction_profiles`, carrying the v7 field-class mode vector:
content, reason, evidence, error, path, and credential all whole-replaced,
metadata preserved. Because it is a normal custom profile rather than an
immutable built-in, you can read it, edit it, or point destinations somewhere
else.

One class differs from v7, and it is a property of the v8 model rather than of
the migration: **identifiers are preserved**. v7 whole-replaced them; no v8
profile redacts them, and a custom profile that tries is rejected at load.
Identifiers are what joins one record to another, so the model reserves that
decision rather than letting a profile sever correlation silently. Nothing v7
redacted as content, reason, evidence, error, path, or credential is revealed
by upgrading.

<Callout type="warn" title="User identifiers are not redacted">
This preserved class includes `user.id`, `defenseclaw.user.id_kind`,
`defenseclaw.user.name`, and `defenseclaw.user.email`. In particular, enabling
`ai_discovery.include_user_email` can export a sensitive cross-system personal
identifier as plaintext; no built-in or custom v8 profile can remove it. Keep
email collection disabled unless every selected destination and your privacy
policy allow it. See [End-user identity](/docs/observability/end-user-identity).
</Callout>
...
materializes their effective behavior into the `none` built-in or a generated
`v7-compatible` custom profile before retiring those inputs. A v8 gateway rejects legacy observability/redaction blocks
