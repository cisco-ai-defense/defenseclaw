...
| `defenseclaw setup observability add\|list\|enable\|disable\|remove\|test\|migrate-otel` | Manage named OTLP telemetry destinations and audit-log sinks; convert flat OTel config once. Distinct from `setup webhook`, which manages notifier webhooks. |
