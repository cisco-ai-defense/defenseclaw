...
Windsurf, GitHub Copilot CLI, Antigravity, OpenCode, Hermes, and OmniGent.
Gemini CLI, OpenHands, OpenClaw, and ZeptoClaw remain unavailable there. Some
...
for the Windows classification and safe examples.
</Callout>
...
| `defenseclaw setup cursor` | Native Windows user-hook alias for Cursor; defaults to observe, while `--mode action` uses event-native deny with `failClosed: true`. Native human approval is not enabled. |
| `defenseclaw setup windsurf` | Native Windows compatibility alias for Devin Desktop legacy Cascade; Devin Local/cloud/ACP are unsupported. Defaults to observe; pass `--mode action` to block only on supported Cascade pre-hooks. |
| `defenseclaw setup geminicli` | Hook setup alias for the continuing enterprise/Google Cloud/paid API-key Gemini CLI product on supported non-Windows platforms; it is excluded from the native Windows release. Defaults to observe, pass `--mode action` to block hook-side. Consumer/free/Google AI Pro/Ultra service ended June 18, 2026. |
