...
| Command | Use it for |
| --- | --- |
| `defenseclaw upgrade [--version X] [--yes]` | Drive a compatible direct upgrade under the installed controller. It verifies artifacts before stopping services, backs up managed state, installs the CLI and gateway, runs release-owned migrations, restarts services, and performs version-bound health checks. A `0.8.3`-or-older built-in command safely refuses a hard-cut target because it cannot learn the two-process `0.8.4` bridge handoff retroactively; use the authenticated current release-owned shell or PowerShell resolver in latest mode for that path. Release `0.8.4` and later require `cosign`, and `--allow-unverified` cannot bypass that boundary. See [Upgrade DefenseClaw](/docs/get-started/upgrade). |
