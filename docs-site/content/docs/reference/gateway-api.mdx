...
The gateway binds to localhost by default. The `gateway.bind: "lan"` shortcut is only meaningful when `defenseclaw setup` puts the gateway in the sandbox profile (binds `0.0.0.0` so the host-side veth pair can reach it).
