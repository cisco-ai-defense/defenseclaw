---
title: Keys
description: The complete credential reference. Where keys live, the resolution order DefenseClaw uses, and the full table of every credential the gateway and CLI know about.
keywords:
  - defenseclaw keys
  - DEFENSECLAW_LLM_KEY
...
| `AGENT_CONTROL_API_KEY` | `agent_control` | Agent Control policy synchronization is enabled. | Use a bucket-scoped, database-managed member key. The same key authenticates the SDK and read-only UI; never use the unscoped bootstrap administrator key on an endpoint. The displayed env name follows `agent_control.api_key_env` when customized. |
...
For Agent Control, prefer the hidden interactive prompt so the secret is not
...
```text
  ...
  · AGENT_CONTROL_API_KEY       agent_control            NOT_USED     unset   n/a
...
- [Setup → Agent Control](/docs/setup/agent-control) — scoped member keys, managed regex sources, and Monitor visibility.
