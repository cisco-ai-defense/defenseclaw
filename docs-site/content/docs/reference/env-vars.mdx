...
| `AGENT_CONTROL_API_KEY` | Bucket-scoped, database-managed member key used by both the SDK and read-only Agent Control UI. |
...
administrator key, but install only the member key on DefenseClaw:

```bash
defenseclaw keys set AGENT_CONTROL_API_KEY
```

The command prompts with hidden input; do not append the secret to the command
line. The member can read its effective buckets and member-owned Monitor
history but cannot edit controls. The synchronizer also needs
`DEFENSECLAW_GATEWAY_TOKEN` for authenticated loopback activation and readback.
