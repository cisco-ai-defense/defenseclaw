...
| `DEFENSECLAW_CONFIG` | **medium** | `${DEFENSECLAW_HOME}/config.yaml` | `any-absolute-path`, `unset` | Override the config.yaml path independently from DEFENSECLAW_HOME when migrations are applied to an explicitly selected configuration file. | Explicit config path selection keeps migrations aligned with the configuration file chosen by the operator or managed service. | `cli/defenseclaw/migrations.py:145` — Migration config path resolver |
