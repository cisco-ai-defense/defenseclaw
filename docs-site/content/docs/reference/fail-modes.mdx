...
plan with `defenseclaw doctor --fix --dry-run`. The real repair runs before the
diagnostic pass, so the summary and exit code describe the repaired state.
...
or DefenseClaw security opt-outs. Supply those through the trusted service
environment instead.
