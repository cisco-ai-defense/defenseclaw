...
# Legacy flat `otel.protocol` / `otel.endpoint` / signal transport blocks are
# translated into a `generic-otlp` destination before runtime validation.
# Preview and persist the conversion with:
