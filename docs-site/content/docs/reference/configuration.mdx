...
# Fresh init enables this by default. Existing configurations are never
# changed implicitly. The shared credential store remains under ~/.s-gw.
...
Changing `credential_protection.enabled` also requires a gateway restart because
the s-gw tokenization service is verified and started with the gateway. Use
`defenseclaw setup credential-protection` for installation and MCP
registration rather than enabling the field by hand. See
[Credential protection](/docs/setup/credential-protection) for coverage and
prerequisites.
