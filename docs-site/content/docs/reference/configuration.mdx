...
[Semantic model routing](/docs/setup/semantic-routing) for the exact lifecycle,
...
| Devin | `~/.config/devin/config.json` or `<workspace>/.devin/hooks.v1.json` |
| Gemini CLI (retired cleanup only) | `~/.gemini/settings.json`; new setup is unavailable. |
