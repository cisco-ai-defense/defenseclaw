...
The path of least resistance is to extend rather than replace.
...
<Steps>
  <Step>
    **Copy the closest pack.**
    ...
    ```bash
    cp -r ./policies/guardrail/default ./policies/my-org
    ```
  </Step>
  <Step>
  ...
  <Step>
    **Lint.**
    ...
    ```bash
    defenseclaw policy validate ./policies/my-org
    ```

    Catches schema errors, missing severities, and overlapping rule IDs.
  </Step>
  <Step>
    **Test against a corpus.**

    ```bash
    defenseclaw policy test ./policies/my-org \
      --fixtures ./test-prompts.jsonl
    ```

    Useful inside CI — diffs the verdicts against a golden file so you catch regressions before you ship.
  </Step>
  <Step>
    **Activate.**

    ```bash
    defenseclaw setup guardrail --rule-pack ./policies/my-org
    ```
  </Step>
</Steps>
...
defenseclaw policy diff default strict
defenseclaw policy validate ./policies/my-org
defenseclaw policy test ./policies/my-org --fixtures fixtures.jsonl
defenseclaw policy activate ./policies/my-org
```

`policy show` prints the active pack and any operator overrides. `policy diff` is the cleanest way to understand the difference between two packs without reading the YAML diff yourself.
