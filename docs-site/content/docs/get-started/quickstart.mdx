...
Windows, choose Codex CLI or Claude Code from the [Native Windows
guide](/docs/get-started/windows); they are the only certified connectors. Other platforms also offer proxy and
additional hook connectors. `quickstart` configures **one** connector. If you
...
```bash
defenseclaw-gateway audit export --limit 200 --output - | jq .
```
