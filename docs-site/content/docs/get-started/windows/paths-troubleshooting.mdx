...
<Accordions>
  ...
  <Accordion title="Codex or Claude Code hooks do not appear">
    Confirm both products run natively as the same Windows user. Run
    `defenseclaw guardrail status` and `defenseclaw doctor`, then re-run the
    supported connector setup in observe mode. Fully restart the agent. Do not
  ...
  <Accordion title="A connector is disabled or status disagrees with configuration">
    ...
    Use connector-scoped enable/disable so the other supported connector is not
  ...
  <Accordion title="Upgrade is refused or interrupted">
    A source not listed in the target release's signed Windows baseline matrix
...
- Run `defenseclaw-gateway status`, `defenseclaw status`, and
  `defenseclaw guardrail status`; require the intended Codex and/or Claude Code
  rows, mode, fail mode, and enabled state.
...
- If both connectors are installed, repeat the harmless prompt once in each;
  one connector's success does not prove the other's hook or OTLP settings.
...
- Restart Codex and Claude Code, then repeat the harmless per-connector event
  check above.
...
- Keep private receipts and backups untouched until the upgraded gateway and
  both configured connectors are healthy.
