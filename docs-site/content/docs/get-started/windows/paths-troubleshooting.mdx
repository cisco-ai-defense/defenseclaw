...
| `%USERPROFILE%\.defenseclaw\backups\` | Managed connector/upgrade recovery material and the latest `config.yaml.before-redaction` pre-change policy backup. | Do not restore or delete manually; it may contain credential-bearing or unredacted policy state. |
