...
| --- | --- | --- |
| `%USERPROFILE%\.local\bin\defenseclaw.cmd` | CLI/TUI launcher for the installed Python environment. | Do not replace or wrap; re-run the installer to repair it. |
| `%USERPROFILE%\.local\bin\defenseclaw-gateway.exe` | Native gateway and lifecycle command. | Do not replace. |
| `%USERPROFILE%\.local\bin\defenseclaw-hook.exe` | No-console hook runner that connector registrations record. | Do not invoke with fabricated credentials or replace. |
| `%USERPROFILE%\.local\bin\defenseclaw-acp.exe` | ACP guard. | Do not replace. |
| `%USERPROFILE%\.defenseclaw\.venv\` | Python environment with DefenseClaw and its scanners, managed by the installer. | Do not install other packages into it. |
| `%USERPROFILE%\.defenseclaw\previous\` | The install the last upgrade replaced, used by `defenseclaw rollback`. | Do not edit; `defenseclaw rollback` swaps it back. |
| `%USERPROFILE%\.defenseclaw\installer\install.ps1` | The installer that performed the current install. | Do not edit; `defenseclaw rollback` runs it. |
| `%USERPROFILE%\.defenseclaw\logs\install-*.log` | One log per install, upgrade, or rollback. | Read locally when troubleshooting. |
...
The Devin connector has no public configuration-home or executable override on
Windows. Connector setup resolves `%APPDATA%\devin` and the fixed
...
The installer and runtime honour `DEFENSECLAW_HOME`; the default data root is
the current profile's `.defenseclaw`. Use the same value for every command,
including `defenseclaw upgrade`, or DefenseClaw will look for a different
install.
...
not consume those ambient process overrides: connector setup bakes its persisted fail
...
check and connector reconciliation are the supported drift/recovery controls.
...
```powershell
& "$env:USERPROFILE\.local\bin\defenseclaw-gateway.exe" status
```

In cmd, quote the executable itself:

```cmd
"%USERPROFILE%\.local\bin\defenseclaw-gateway.exe" status
```
...
<Accordions>
  <Accordion title="The command is not recognized or the wrong version runs">
    ...
    `defenseclaw version`. Run the install command again if the launcher is missing.
    Do not prepend an unpacked source checkout or embedded runtime directory as
    a permanent workaround.
  </Accordion>
  <Accordion title="An install, upgrade, or rollback fails">
    Read the newest `%USERPROFILE%\.defenseclaw\logs\install-*.log`. A failed
    upgrade restores the previous install automatically and keeps the failed
    attempt in `%USERPROFILE%\.defenseclaw\.failed-<time>`. Close the TUI and
    other DefenseClaw commands if the installer reports files in use, then run
  ...
  <Accordion title="The gateway or watchdog is not running">
    Run `defenseclaw-gateway status` and `defenseclaw-gateway watchdog
    status`, then `defenseclaw-gateway start`. There is no sign-in startup
    entry; a hook also starts a stopped gateway on its first call. The watchdog
    runs only when `gateway.watchdog.enabled` is configured and is not a
    Windows service.
  </Accordion>
  ...
  <Accordion title="An upgrade is interrupted">
    Run `defenseclaw upgrade` (or the install command) again after closing the
    TUI and other DefenseClaw commands. The installer is safe to re-run; a
    failed attempt has already restored the previous install. To go back a
    version, run `defenseclaw rollback`.
  </Accordion>
...
- Run `defenseclaw version`, `defenseclaw migrate --check`, and
  `defenseclaw config validate`.
...
- Confirm the command path still resolves to `%USERPROFILE%\.local\bin`,
...
- Confirm each configured connector has loaded its configuration, restarting
  ...
  event check above. For Devin, use the admitted native CLI in the same Windows
  user session; WSL or a shell substitute is not valid evidence.
