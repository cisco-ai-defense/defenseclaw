...
The PowerShell installer downloads the Windows release ZIP for your architecture, verifies it against the release checksums, installs `defenseclaw-gateway.exe` and `defenseclaw-hook.exe`, creates the CLI shim, and adds `%USERPROFILE%\.local\bin` to the user `PATH`. It also installs `uv` when needed so the Python CLI can run from the per-user virtualenv.
...
matrix, multi-connector setup, file locations, and troubleshooting.
...
On Windows, read the same tree under `%USERPROFILE%`: `%USERPROFILE%\.defenseclaw\` for config, audit state, hooks, and the virtualenv; `%USERPROFILE%\.local\bin\` for `defenseclaw.cmd`, `defenseclaw-gateway.exe`, and the no-console `defenseclaw-hook.exe` launcher.
...
<Cards>
  <Card title="Native Windows" href="/docs/get-started/windows" description="PowerShell install, native hook connectors, verification, and troubleshooting." />
