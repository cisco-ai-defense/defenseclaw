...
installation command after the authenticated Setup artifact is downloaded.
...
Set `CONNECTOR` to a supported native Windows setup name from the
[connector platform table](/docs/connectors#platform-support), or use
`CONNECTOR=none` to configure later. The setup executable contains the offline
payload required for install;
...
Native Windows uses connector-native hook, plugin, or awaited-policy lanes
rather than the OpenClaw/ZeptoClaw proxy path. See
[Native Windows](/docs/get-started/windows) for repair, servicing, uninstall,
...
The published artifacts and checked-in source metadata on this page are release
`0.8.10`. The protected release workflow revalidates and stamps an isolated
build checkout before producing artifacts. A source build reports the exact
checked-in source version but must not be represented as a signed published
artifact.
</Callout>
