...
DefenseClaw provisions 14 native Grafana dashboards plus one optional external
producer view for ShadowClaw. Each native board has one job: start broad,
select a connector, agent, rule, or execution, and then move to the detail
surface that owns that question. The catalog avoids duplicating the same
analysis across several boards.
...
| **ShadowClaw — Shadow AI Detections** (`shadowclaw-shadow-ai`) | What AI-provider and agent activity did the independent endpoint sensor observe? | SOC, endpoint security | Loki | Optional ShadowClaw view over its own OTLP records, including process, provider, risk, and host-plane evidence beside DefenseClaw discovery and findings. |

## Retain, consolidate, and remove decisions

- **Retain the 14 native boards above.** Each owns a distinct operator question or a
  deliberate overview → detail drill-down; none is a renamed duplicate.
- **The ShadowClaw board is an external producer view.** It is empty unless a
  ShadowClaw sensor exports to the local Collector, and its fields are not part
  of DefenseClaw's canonical v8 telemetry contract.
