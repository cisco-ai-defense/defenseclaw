...
DefenseClaw provisions 14 native Grafana dashboards plus one bundled
...
| **Shadow AI Discovery Board** (`shadow-ai-discovery`) | What endpoint activity did the bundled Phase 1 observer see? | SOC, endpoint security | Loki + Prometheus | Observation-only view over bounded process identity, coverage, exporter health, and resource usage beside DefenseClaw telemetry. |
...
- **The Shadow AI Discovery Board is a bundled producer view.** It is empty
  until the endpoint observer exports to the local Collector, and its fields
  are not part of DefenseClaw's canonical v8 telemetry contract.
