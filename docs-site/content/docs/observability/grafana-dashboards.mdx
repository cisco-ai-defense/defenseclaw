...
endpoint-telemetry view. Each native board has one job: start broad,
...
| **Shadow AI Discovery Board** (`shadow-ai-discovery`) | What AI-provider and agent activity did the independent endpoint sensor observe? | SOC, endpoint security | Loki | Optional view over external OTLP records, including process, provider, risk, and host-plane evidence beside DefenseClaw discovery and findings. |
...
- **The Shadow AI Discovery Board is an external producer view.** It is empty
  unless a compatible endpoint sensor exports to the local Collector, and its
  fields are not part of DefenseClaw's canonical v8 telemetry contract.
