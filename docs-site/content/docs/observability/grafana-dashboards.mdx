...
time-range-preserving drill-down links. Session and spawn anchors may use the
prior 24 hours to keep selected-range boundary edges renderable; a recovered spawn
appears only when that child has graph-eligible activity in the selected range.
