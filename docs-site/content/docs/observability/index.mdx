...
<DefenseClawDemo scenario="observability-correlation" />
...
<Cards>
  ...
  />
  <Card
    title="Galileo (Cloud + self-hosted)"
    href="/docs/observability/galileo"
    description="First-class traces-only OTLP destination with a guided wizard, canary test, and native coexistence with the local stack."
  />
  <Card
    title="Agent360 drill-down"
    href="/docs/observability/agent360"
    description="Visual per-agent guide to lifecycle, models, tools, tokens, cost, security decisions, topology, and Tempo trace drill-down."
  />
  <Card
    title="Grafana dashboard catalog"
    href="/docs/observability/grafana-dashboards"
    description="Purpose, ownership, data sources, drill-down paths, and empty-state semantics for every bundled dashboard."
  />
</Cards>
...
Destination names are identities: a new name adds a route, while an existing
name updates only that route. Inspect and preview before writing:
...
The list distinguishes `otel` from `audit_sinks` and shows kind, enabled state,
...
<Accordions>
  ...
  <Accordion title="Can I send to my own OTLP collector?">
    Yes. Use `defenseclaw setup observability add otlp --endpoint ...`, or pass `--endpoint` to `setup local-observability up`. Named entries under `otel.destinations[]` can fan the same telemetry out to multiple OTLP gRPC/HTTP backends. See the [Galileo guide](/docs/observability/galileo) for a local-plus-cloud example.
  </Accordion>
