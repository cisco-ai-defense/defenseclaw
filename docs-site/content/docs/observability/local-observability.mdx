...
<Accordions>
  <Accordion title="Grafana shows no data">
    Run `defenseclaw setup local-observability up` again so the host-mounted dashboards and collector config are refreshed from the bundle, then send a synthetic event with `defenseclaw setup observability test` and check the collector logs (`defenseclaw setup local-observability logs --service otel-collector --follow`). Most remaining "no data" issues are the collector not being reachable from the gateway, not a Grafana problem.
  </Accordion>
