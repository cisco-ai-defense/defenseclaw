...
keywords:
  ...
  - native Windows Docker Desktop observability
---
...
<Callout type="info">The shared Python/Compose controller supports macOS,
Linux, and native Windows x64. The certified Windows path runs the CLI from
PowerShell or cmd and requires Docker Desktop Linux containers through the
Hyper-V backend on Windows Pro, Enterprise, or Education. WSL commands are not
used; per-user and WSL-only Docker Desktop installations are outside this
no-WSL certification.</Callout>
...
<Steps>
  <Step>
    **Start the stack.**
    ...
    The packaged Python controller invokes Docker Compose with an explicit
    project directory, Compose file, and project name, then verifies OTLP,
    Grafana, Prometheus, Tempo, and Loki readiness before the CLI enables the
    local destination in `config.yaml`.
  </Step>
  <Step>
    **Open Grafana.** [http://localhost:3000](http://localhost:3000) — the loopback-only developer stack provisions anonymous Admin access and disables the login form.
  </Step>
  <Step>
    **Generate traffic.** Run any DefenseClaw-managed agent, or send a synthetic event to every configured sink:
    ...
    The `test` subcommand sends a probe to the named `local-observability`
    destination. Run `defenseclaw setup observability list` first if you renamed
    it. Test other destinations individually by name; notifier webhooks use
...
| **Runtime & Reliability** (`defenseclaw-runtime`) | Process, SQLite, exporter, audit-sink, and canonical gateway-error health. |
...
| **Agent360** (`defenseclaw-agent-360`) | Automatic Agent Directory plus one-click per-agent/tree lifecycle, executions, descendants, model calls, tools/websites, inputs/outputs, reported tokens/cost, security decisions, Tempo waterfall, and agent/subagent/model/tool topology. |
...
| `trace_id` | Direct Loki/Prometheus exemplar pivot into the Tempo waterfall. |
...
token, reported-cost totals, and the dynamic agent/subagent/model/tool graph.
Loki supplies compact lifecycle/model/tool/network/security summaries with the
full event available in log details. Tempo supplies the complete request
waterfall; the trace view also exposes Grafana's built-in span graph. Select a
row in **Recent traces** to populate the waterfall. Token and cost panels only
...
tour of every section, the agent/model/tool topology, trace drill-down, and the
...
<Accordions>
  <Accordion title="Grafana shows no data">
    Run `defenseclaw setup local-observability up` again so the host-mounted dashboards and collector config are refreshed from the bundle, then send a synthetic event with `defenseclaw setup observability test local-observability` and check the collector logs (`defenseclaw setup local-observability logs --service otel-collector --follow`). Most remaining "no data" issues are the collector not being reachable from the gateway, not a Grafana problem.
  </Accordion>
  <Accordion title="Port already in use">
    Grafana 3000, Loki 3100, Tempo 3200, OTLP 4317/4318, and Prometheus 9090 are pinned to loopback. Compose fails truthfully if a foreign process owns a port. DefenseClaw never deletes a foreign same-name container; resolve the reported collision and re-run `up`.
  </Accordion>
  <Accordion title="Reverting config.yaml">
    `down` preserves config by default. Run `defenseclaw setup local-observability down --disable-config` to stop the stack and disable its local OTel destination and matching audit sink.
  </Accordion>
</Accordions>
