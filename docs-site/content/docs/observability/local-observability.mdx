...
<Steps>
  ...
  <Step>
    **Generate traffic.** Run any DefenseClaw-managed agent, or seed events with:

    ```bash
    defenseclaw doctor --emit-test-events
    ```
  </Step>
  <Step>
...
| Flag | What it does |
| --- | --- |
| `--endpoint` | OTLP gRPC endpoint the gateway writes into config.yaml. Defaults to `127.0.0.1:4317`. |
...
| `--service-name` | Resource attribute set on every emitted span. |
| `--with-audit-sink` / `--no-audit-sink` | Also (or don't) add an `audit_sinks: otlp_logs` entry so audit events flow as OTel logs. |
...
| `--timeout` | Seconds to wait for healthchecks. |
...
<Files>
  <Folder name="grafana/dashboards" defaultOpen>
    <File name="overview.json" />
    <File name="guardrail-verdicts.json" />
    <File name="admission-and-scanners.json" />
    <File name="llm-judge.json" />
    <File name="hitl.json" />
  </Folder>
</Files>
...
```bash
defenseclaw setup local-observability down --purge
```

<Callout type="warn">
`--purge` is destructive — your historical traces, logs, and metrics
go with it. Splunk HEC and JSONL sinks are unaffected.
</Callout>
...
defenseclaw setup local-observability logs grafana
```

`status` shows container health and the resolved OTLP endpoint. `logs <service>` tails one of the five containers — useful when Grafana isn't picking up dashboards or the OTel collector is dropping spans.
...
<Accordions>
  <Accordion title="Grafana shows no data">
    Generate test events: `defenseclaw doctor --emit-test-events`. Then check the OTel collector container logs (`defenseclaw setup local-observability logs otel-collector`) — most "no data" issues are the collector not being reachable from the gateway, not a Grafana problem.
  </Accordion>
  <Accordion title="Port already in use">
    Grafana 3000, Loki 3100, Tempo 3200, OTLP 4317/4318, Prometheus 9090. Kill the conflicting service or shut the stack down with `down` and re-run with `--port-shift N` to add an offset.
  </Accordion>
