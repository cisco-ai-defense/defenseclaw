...
The `defenseclaw keys` command does **not** read or write the macOS Keychain. Its on-disk store remains `~/.defenseclaw/.env`, and transient overrides go through `os.environ`. The optional [s-gw credential broker](/docs/setup/credential-protection) is a separate integration with its own encrypted credential store and approval workflow; DefenseClaw does not migrate existing `keys` values into it.
</Callout>
