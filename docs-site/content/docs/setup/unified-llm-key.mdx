...
Click prompts for the value with `hide_input=True`, then writes it to the
current user's DefenseClaw dotenv file: `~/.defenseclaw/.env` on macOS/Linux
or `%USERPROFILE%\.defenseclaw\.env` on Windows. POSIX installs enforce mode
`0o600`; native Windows installs replace inherited access with an owner-private
DACL. The value is masked in stdout (`abcd…wxyz`), masked in audit, and never
echoed.
...
| Priority | Source | macOS/Linux example | Windows PowerShell example |
| --- | --- | --- | --- |
| 1 | Process environment | `export DEFENSECLAW_LLM_KEY='sk-…'` | `$env:DEFENSECLAW_LLM_KEY = 'sk-…'` |
| 2 | User dotenv | `~/.defenseclaw/.env` | `%USERPROFILE%\.defenseclaw\.env` |
| 3 | Unset | Required keys are marked missing; optional keys are tolerated. | Same behavior. |
...
<Callout title="No operating-system keychain or external secret manager">
DefenseClaw does **not** read or write macOS Keychain, Windows Credential
Manager, 1Password, Vault, AWS Secrets Manager, or another external secret
store. The user dotenv file is the only on-disk credential store. To source a
secret elsewhere, inject it into the process environment with `export` on
macOS/Linux or `$env:NAME = ...` in PowerShell.
</Callout>
...
```powershell
$names = 'DEFENSECLAW_LLM_KEY', 'CISCO_AI_DEFENSE_API_KEY', 'SPLUNK_ACCESS_TOKEN'
foreach ($name in $names) {
  $value = (Get-Item "Env:$name").Value
  defenseclaw keys set $name --value $value
}
defenseclaw keys check
```
...
| Where | What | Notes |
| --- | --- | --- |
| User-home `.defenseclaw/.env` | Canonical credential store | Atomic tmp+rename writes. POSIX uses `0o600`; Windows uses an owner-private DACL. No comments or metadata. |
| User-home `.defenseclaw/audit.db` | Action log entry per `keys set` | Records `actor=cli:operator action=config.update target=dotenv:<ENV>` with `before/after = had_value`. The value is **never** logged. |
| User-home `.defenseclaw/config.yaml` | The `*_env` references only | The actual secret never enters `config.yaml`. |
| Process environment | Highest-priority lookup | `export` on macOS/Linux or `$env:NAME = ...` in PowerShell wins over the dotenv copy. Useful for one-shot debugging. |
...
| --- | --- | --- |
| Hook-based | Codex, Claude Code, Hermes, Cursor, Devin, Copilot CLI, OpenHands, Antigravity, OpenCode, Amp, OmniGent | The agent talks to its own LLM directly. DefenseClaw runs as a *judge only*: it inspects prompts/responses and emits verdicts. The unified key is only needed when `guardrail.judge.enabled: true`. |
...
| Hook (Hermes, Cursor, Codex, OpenCode, Amp, OmniGent, Claude Code, Devin, Copilot, OpenHands, Antigravity) | **Configures DefenseClaw's judge/aux model only.** The agent talks to its own model directly — those calls are never routed through or inspected by DefenseClaw, so a custom provider cannot change the agent's model. |
