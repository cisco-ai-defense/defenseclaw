...
DefenseClaw integrates Cisco's open-source [`cisco-ai-skill-scanner`](https://github.com/cisco-ai-defense/skill-scanner) so operator-installed and modified skills get a deterministic + LLM-assisted review before an agent is allowed to use them. Verdicts feed the `skill_actions` admission policy by severity bucket, and the install watcher quarantines new skills until the scan resolves. Exact proven vendor bundles remain visible as discovery-only inventory and do not enter scanner or enforcement paths.
...
For automation, add `--json`. Batch JSON remains a single top-level array for
...
Codex's personal and project skill directories both use the shared
`.agents/skills` namespace. Path alone cannot distinguish a protected install
from an attacker-controlled repository entry, so CodeGuard at either Codex
location remains scan-required unless stronger provenance is available.
Codex also discovers operator-installed skills under `$CODEX_HOME/skills`;
those remain scan-required. Only the exact vendor cache at
`$CODEX_HOME/skills/.system` is discovery-only and excluded from every scanner
and enforcement entrypoint. An unrelated `.system` directory receives no
exemption.

Hermes mixes installer-synced and operator-authored skills under
`$HERMES_HOME/skills`. DefenseClaw grants discovery-only status only when the
`.bundled_manifest` hash, installed copy, and matching
`$HERMES_HOME/hermes-agent/skills` source tree all agree. Modified, untracked,
linked, oversized, or otherwise unverifiable copies remain scan-required.
