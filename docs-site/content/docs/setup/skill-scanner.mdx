...
`cisco-ai-skill-scanner` ships as an optional dependency. The wrapper detects it lazily so a vanilla `pip install defenseclaw` works without it:
...
If it's missing when you run a scan, DefenseClaw prints the install hint and exits cleanly — it does not crash the gateway.
