...
description: Scan configured connector skills, including Scout skill surfaces, before an agent can execute them. DefenseClaw wraps cisco-ai-skill-scanner and writes its verdicts into the same skill_actions admission policy as the watcher.
keywords:
...
- **Local skill name** (`my-skill`) — resolves through configured connector skill directories. On a multi-connector install, a bare name checks matching connector copies; pass `--connector <name>` to force one connector's skill source. Scout read surfaces include `~/.copilot/bundled-skills`, `~/.copilot/m-skills`, and `~/.copilot/skills`.
...
Use `--connector scout` to target Scout's local skill surfaces even though Scout is not a guardrail-wired connector.
  </Tab>
