...
<Cards>
  ...
  <Card title="setup windsurf" href="/docs/setup/guardrail/aliases" description="Adds or reconfigures Devin Desktop (stable ID: windsurf) for Cascade hooks plus documented MCP, skills, rules, and instructions." />
...
These pages explain operator policy and deployment architecture; they are not
additional `defenseclaw setup` verbs.
...
<Callout title="Redaction is configured directly as v8 policy">
There is no `setup` toggle for redaction. Edit
`observability.destinations[].routes[].selector.buckets` and
`observability.redaction_profiles`, then run `defenseclaw config validate`,
