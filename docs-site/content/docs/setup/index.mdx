...
<Cards>
  ...
  <Card title="setup copilot" href="/docs/setup/guardrail/aliases" description="Pins claw.mode=copilot and writes the .github/hooks command hooks." />
...
<Cards>
  <Card
    title="setup keys / unified-llm-key"
    href="/docs/setup/unified-llm-key"
    description="Stash DEFENSECLAW_LLM_KEY (and any per-component overrides) in ~/.defenseclaw/.env. List, set, fill-missing, check."
  />
  <Card
    title="setup webhook"
...
| `setup splunk` | yes | `--non-interactive` + flags | Token always read from env var (`--hec-token-env`). |
| `setup local-observability` | yes | flags + `--non-interactive` | One-shot bring-up; `--down` tears the stack back down. |
| `setup skill-scanner` / `setup mcp-scanner` | yes | `--non-interactive` + flags | Per-feature toggles; SDKs ship as pip extras. |
...
Next steps: [`defenseclaw setup guardrail`](/docs/setup/guardrail) is the right starting point if you have not run it yet. Already running? [`defenseclaw setup unified-llm-key`](/docs/setup/unified-llm-key) is the most common follow-up — it unlocks the LLM judge and the LLM-backed scanners.
