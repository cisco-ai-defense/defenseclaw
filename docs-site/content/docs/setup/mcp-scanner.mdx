...
- **Local stdio server name** — described by connector-native configuration (`mcpServers` JSON for Claude Desktop, Cursor, OpenClaw, Gemini, and Copilot; `[mcp_servers]` TOML for Codex). DefenseClaw spins each server up in a sandbox, lists its tools, and records every advertised description, parameter schema, and example.
