...
The separate `macOS Enterprise Packaging` CI job executes the shipped `packaging/launchd/install-enterprise.sh` on a disposable macOS runner. It creates a non-login `defenseclaw` identity, verifies that a permissive source config is installed as `root:defenseclaw` mode `0640`, widens and changes the installed config owner to prove a second install repairs drift, and confirms that a symlink destination is rejected without modifying its decoy target.
