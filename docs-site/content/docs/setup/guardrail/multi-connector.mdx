...
<Video
  src="multi-connector"
  caption="One gateway governing Claude Code and Antigravity at once. These examples are tool calls, so each reaches its connector's documented synchronous PreToolUse gate: a benign `echo` flows through, a HIGH-severity `chmod 777` finding can request native confirmation, and a CRITICAL credential-read finding can return deny. Antigravity ask/deny authority does not extend to its other four lifecycle events. Every verdict is attributed per connector in the Splunk pane. The demo closes by deleting each agent's hook config and watching DefenseClaw's self-heal detect the tamper and reinstall the hooks automatically (connector-hook-tampered → connector-hook-repaired)."
/>
