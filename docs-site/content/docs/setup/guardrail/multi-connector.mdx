...
Omit `--connector` and the same verbs operate on the **global default** that every connector inherits. `--connector` is rejected on a single-connector install — there is only one posture to set. See [Reference → CLI](/docs/reference/cli#guardrail) for the full command group.
