...
```mermaid
flowchart LR
    Admin["Administrator or MDM"] --> RootConfig["Root-owned config and policy"]
    Admin --> RootBinary["Root-owned binaries and service definitions"]
    Admin --> Manifest["Root-owned target allow-list"]
    RootConfig --> Gateway["DefenseClaw gateway service"]
    RootBinary --> Gateway
    Manifest --> Guardian["Privileged hook guardian"]
    Guardian --> Ledger["Root-owned authorization ledger"]
    Guardian --> UserHooks["User-owned native agent hooks"]
    UserHooks --> ScopedToken["Connector-scoped raw token"]
    ScopedToken --> HookAPI["Loopback connector hook route"]
    HookAPI --> Gateway
    Ledger --> Readiness["Managed readiness and enforcement health"]
    Gateway --> Readiness
    Agent["Codex, Claude Code, Cursor, or other agent"] --> UserHooks
```
