...
| --- | --- | --- | --- |
| Binaries | `/opt/defenseclaw/bin` | `/opt/cisco/secureclient/defenseclaw/bin` | root/admin |
| Config | `/etc/defenseclaw/config.yaml` | `/opt/cisco/secureclient/defenseclaw/etc/config.yaml` | root/admin |
| Runtime state | `/var/lib/defenseclaw` | `/opt/cisco/secureclient/defenseclaw/runtime` | root |
| Logs | `/var/log/defenseclaw` | `/Library/Logs/Cisco/SecureClient/DefenseClaw` | root |
| Linux service | `/etc/systemd/system/defenseclaw-gateway.service` | n/a | root |
| macOS service | n/a | `/Library/LaunchDaemons/com.cisco.secureclient.defenseclaw.plist` | root |
...
The gateway daemon runs as root (uid 0). No dedicated service user is created — the managed cloud auth provider requires root to read and re-perm its on-disk credential store.
