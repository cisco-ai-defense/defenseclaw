...
2. **The executable and service definition are administrator-owned.** A standard user cannot replace `/opt/defenseclaw/bin/defenseclaw-gateway`, edit the systemd unit, replace `/opt/cisco/secureclient/defenseclaw/bin/defenseclaw-gateway`, unload a system LaunchDaemon, replace `C:\Program Files\Cisco\DefenseClaw\bin\defenseclaw-gateway.exe`, or change/delete the Windows SCM services.
...
| --- | --- | --- |
| Gateway, native hook, and optional CLI binaries | `C:\Program Files\Cisco\DefenseClaw\bin` | Administrators/LocalSystem own and write; service identities receive only the access they need. |
| Managed config | `C:\ProgramData\Cisco\DefenseClaw\etc\config.yaml` | Administrators/LocalSystem write; the gateway service can read. |
| Runtime state and service-side scoped credentials | `C:\ProgramData\Cisco\DefenseClaw\runtime` | Gateway virtual account and LocalSystem; standard users have no read or write grant. |
| Logs | `C:\ProgramData\Cisco\DefenseClaw\logs` | Service identities write; grant support-reader access separately if required. |
| Guardian manifest | `C:\ProgramData\Cisco\DefenseClaw\hook-guardian\targets.yaml` | Administrators/LocalSystem write; guardian reads. |
| Trusted authorization ledger | `C:\ProgramData\Cisco\DefenseClaw\hook-guardian-state\protected_targets.json` | LocalSystem/Administrators only; gateway receives the narrow read access required for readiness. |
| Deployment metadata | `C:\ProgramData\Cisco\DefenseClaw\install\deployment.json` | Installer-owned transaction and servicing state. |
...
| Agent-control evidence | `C:\ProgramData\Cisco\DefenseClaw\install\agent-application-control-attestation.json` | Protected schema-v2 application-control and Claude-precedence evidence, plus a separate Codex trusted-launcher result. |
...
config_version: 7
deployment_mode: managed_enterprise
data_dir: 'C:\ProgramData\Cisco\DefenseClaw\runtime'
audit_db: 'C:\ProgramData\Cisco\DefenseClaw\runtime\audit.db'
judge_bodies_db: 'C:\ProgramData\Cisco\DefenseClaw\runtime\judge_bodies.db'
plugin_dir: 'C:\ProgramData\Cisco\DefenseClaw\runtime\plugins'
policy_dir: 'C:\ProgramData\Cisco\DefenseClaw\runtime\policies'
gateway:
  device_key_file: 'C:\ProgramData\Cisco\DefenseClaw\runtime\device.key'
  api_bind: 127.0.0.1
  api_port: 18970
  config_reload:
    mode: restart
...
`Cisco\DefenseClaw-Cert` roots, and a required certification CODEX_HOME
...
$Stage = 'C:\ProgramData\Cisco\DefenseClaw-Staging\<release-id>'
...
$Manifest = 'C:\ProgramData\Cisco\DefenseClaw\hook-guardian\targets.yaml'
...
Get-Acl 'C:\Program Files\Cisco\DefenseClaw\bin\defenseclaw-gateway.exe' |
  Format-List Owner, Sddl
Get-Acl 'C:\ProgramData\Cisco\DefenseClaw\etc\config.yaml' |
  Format-List Owner, Sddl
Get-Acl 'C:\ProgramData\Cisco\DefenseClaw\hook-guardian\targets.yaml' |
  Format-List Owner, Sddl
Get-Acl 'C:\ProgramData\Cisco\DefenseClaw\hook-guardian-state\protected_targets.json' |
  Format-List Owner, Sddl
```
...
```powershell
$InstalledCLI = 'C:\Program Files\Cisco\DefenseClaw\bin\defenseclaw.exe'
...
C:\ProgramData\Cisco\DefenseClaw\install\codex-requirements-ownership.json
C:\ProgramData\Cisco\DefenseClaw\install\codex-requirements-acl-backup.json
```
...
Codex must be at least 0.131.0, but stock signed Codex 0.144.3 is intentionally
...
The installer refuses symlinked sources or managed destinations, verifies the root-owned macOS system path chain, unloads any existing DefenseClaw jobs, and installs through same-directory temporary files. Before launchd is allowed to start either job, it verifies these postconditions:
