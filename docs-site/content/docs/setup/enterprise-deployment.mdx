...
| Compromised guardian process | Root on Linux/macOS or LocalSystem on Windows | Can repair enrolled user-home footprints and guardian state. Risk is reduced through the protected target manifest, trusted path checks, per-target credential changes or impersonation, narrow service arguments, and platform filesystem controls. |
| Compromised enumerator process | LocalSystem on Windows | Can discover eligible profiles, publish the protected target manifest, and grant the exact gateway SID bounded read/execute/traverse access on inventory directories. Its product contract does not repair hook footprints or write the authorization ledger. Risk is reduced through interactive-SID/profile filters, the managed-hook connector filter, protected atomic publication, and fixed DACL targets. |
