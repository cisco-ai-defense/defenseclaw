...
On macOS, a fresh Codex or Claude Code install must pair the reviewed version
with the exact canonical native executable selected by the administrator. Do
not supply a wrapper, symlink, application-discovery result, or `PATH` name:

```bash
sudo DEFENSECLAW_CONFIG=/opt/cisco/secureclient/defenseclaw/etc/config.yaml \
  /opt/cisco/secureclient/defenseclaw/bin/defenseclaw-gateway enterprise hooks install \
  --user alice \
  --connector codex \
  --agent-version "codex-cli 0.146.0" \
  --agent-executable /Users/alice/.npm-global/lib/node_modules/@openai/codex/node_modules/@openai/codex-darwin-arm64/vendor/aarch64-apple-darwin/bin/codex \
  --json
```

The macOS guardian fails closed before connector setup when either half of
this pair is missing or inconsistent. It revalidates the native image's
custody, signer, architecture, quarantine/ACL state, bounded digest, and
reported version before installing hooks. A repair with no explicit pair may
reuse only the exact identity already sealed in the protected contract lock;
it never falls back to discovery or `PATH`.
...
For macOS Codex and Claude Code targets, each enabled manifest row must pair
`agent_version` with the exact canonical `agent_executable`. For example:
...
targets:
  - user: alice
    ...
    agent_executable: "/Users/alice/.npm-global/lib/node_modules/@openai/codex/node_modules/@openai/codex-darwin-arm64/vendor/aarch64-apple-darwin/bin/codex"
  - user: alice
    connector: claudecode
...
The packaged renderer emits a protected row only when it finds a passively
paired version and native executable; it does not execute an unvalidated
client during discovery. Hand-authored MDM manifests must follow the same
pairing rule. A fresh Darwin Codex or Claude Code target without this evidence
is rejected rather than discovered dynamically.

OpenHands is intentionally not auto-rendered or admitted by the macOS
enterprise guardian. Its current standalone image is unsigned, so a path and
self-computed receipt hash are not publisher provenance. Keep OpenHands out of
the guardian manifest until the deployment boundary supplies an
administrator-reviewed release-digest policy; DefenseClaw fails closed instead
of trusting a user-owned version directory. This remains unsupported for
macOS enterprise hooks even though the connector is available as a preview;
