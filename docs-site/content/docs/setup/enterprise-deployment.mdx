...
| Compromised guardian or enumerator process | Root on Linux/macOS or LocalSystem on Windows | Can repair enrolled user-home footprints and guardian state. Risk is reduced through the protected target manifest, Windows interactive-SID/profile filters, trusted path checks, per-target credential changes or impersonation, narrow service arguments, and platform filesystem controls. |
...
- [ ] Guardian targets are current and authorized: Linux/macOS manifests contain only explicit approved pairs; Windows enumerator output contains only expected interactive SIDs, configured connector families, and discovered supported versions.
