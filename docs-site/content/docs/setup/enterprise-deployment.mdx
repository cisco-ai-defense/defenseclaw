...
<Flow
  direction="TB"
  compact
  caption="Administrator policy and target authorization remain outside the agent user's authority. The gateway evaluates hook traffic; the guardian repairs only integrations present in the protected target manifest."
>
