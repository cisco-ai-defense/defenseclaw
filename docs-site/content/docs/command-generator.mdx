...
<Cards>
  <Card title="Connector picker" description="All twelve first-class connectors — proxy (OpenClaw, ZeptoClaw) and hooks (Claude Code, Codex, Cursor, Windsurf, Gemini CLI, GitHub Copilot CLI, Hermes, OpenCode, OpenHands, Antigravity). Each card surfaces whether the connector supports native ask and fail-closed enforcement so HITL decisions are informed." />
...
2. **`defenseclaw setup guardrail` is always the verb.** The hook-only aliases (`defenseclaw setup claude-code`, etc.) only configure observability and accept a much smaller flag surface; the guardrail verb works for all twelve connectors and is the path documented for CI.
