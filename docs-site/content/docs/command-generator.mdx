...
2. **`defenseclaw setup guardrail` is always the verb.** The hook-only aliases (`defenseclaw setup claude-code`, etc.) only configure observability and accept a much smaller flag surface; the guardrail verb works for all thirteen connectors and is the path documented for CI.
