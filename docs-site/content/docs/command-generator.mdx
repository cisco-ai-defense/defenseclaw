...
2. **`defenseclaw setup guardrail` is always the generated verb.** Connector aliases (`defenseclaw setup claude-code`, `defenseclaw setup omnigent`, etc.) default to observe mode but also accept action mode and a smaller connector-specific flag surface; the guardrail verb works for all thirteen connectors and remains the documented CI path.
