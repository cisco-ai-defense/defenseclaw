...
description: Build a Bash or PowerShell `defenseclaw setup guardrail` command for any connector. Pick mode, scanner backend, detection strategy, rule pack, HITL behaviour, and every advanced knob; copy the result straight into your terminal or CI pipeline.
keywords:
...
<Callout>
Generated commands are display-only. Nothing on this page runs against your machine. Select **Bash / zsh** for macOS or Linux, or **PowerShell** for native Windows, then copy the result into that host's terminal.
</Callout>
...
<Cards>
  <Card title="Connector picker" description="All thirteen active first-class connectors — proxy (OpenClaw, ZeptoClaw) and direct-to-upstream hooks or policy plugins (Claude Code, Codex, Cursor, Devin, GitHub Copilot CLI, Hermes, OpenCode, Amp, OmniGent, OpenHands, Antigravity). Each card surfaces native ask, fail-closed, and selected-shell platform support. Native-Windows-unsupported choices are disabled in PowerShell mode, while degraded support produces an explicit warning." />
  <Card title="Shell + scanner backend" description="Choose Bash/zsh or PowerShell output, then configure --scanner-mode (local / remote / both). Picking remote or both surfaces the Cisco endpoint, API key env var, and timeout fields, and emits the matching export or $env: assignment." />
...
2. **`defenseclaw setup guardrail` is always the generated verb.** Connector aliases (`defenseclaw setup claude-code`, `defenseclaw setup amp`, etc.) default to observe mode but also accept action mode and a smaller connector-specific flag surface; the guardrail verb works for all thirteen connectors and remains the documented CI path.
3. **Quoting matches the selected shell.** Bash/zsh output uses POSIX single-quote escaping and `\\` continuations. PowerShell output doubles embedded apostrophes inside single-quoted arguments, emits `$env:NAME = 'value'`, and uses backtick continuations. Placeholder credential values are quoted in both modes so angle brackets are never interpreted as shell operators.
4. **Conflicting choices surface as warnings; unsupported platforms are rejected.** If you ask for `--human-approval` in observe mode, the flag is omitted (matching CLI behaviour) and a note explains why. In PowerShell mode, connectors without a native Windows path are disabled and an already-selected unsupported connector becomes non-copyable until you choose a supported connector or return to Bash/zsh. Degraded Windows support remains selectable with a prominent boundary warning.
