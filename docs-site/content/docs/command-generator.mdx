...
<Cards>
  <Card title="Connector picker" description="All fourteen first-class connectors — proxy (OpenClaw, ZeptoClaw) and direct-to-upstream hooks or policy plugins (Claude Code, Codex, Cursor, Devin Desktop legacy Cascade, Gemini CLI, GitHub Copilot CLI, Hermes, OpenCode, Amp, OmniGent, OpenHands, Antigravity). The windsurf alias covers Cascade only, not Devin Local. Each card surfaces whether the connector supports native ask and fail-closed enforcement so HITL decisions are informed." />
  ...
  <Card title="Advanced knobs" description="--port, --block-message, --workspace, --restart / --no-restart, --verify / --no-verify, plus --disable (teardown). Redaction is configured separately as v8 route/profile policy; --disable collapses the command to the guardrail-off verb." />
</Cards>

<Callout title="Redaction is v8 policy, not a setup flag">
Choose destination buckets with
`observability.destinations[].routes[].selector.buckets` and define reusable
policy under `observability.redaction_profiles`. After editing `config.yaml`, run
