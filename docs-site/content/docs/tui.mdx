...
| `0` | Setup | Connector, gateway, and credential-protection setup |
...
Setup's **Credential Protection** config section shows the current
`credential_protection.enabled` state and explains whether the active connector
has a prompt-tokenization path through the DefenseClaw proxy. Use the command
palette actions to set up s-gw, check its safe status, open its authenticated approval console, or
disable the integration; those commands keep module verification and connector
