...
<Flow
  direction="TB"
  caption="The connector contract establishes trust. CEL never trusts a payload to declare itself structured, and every candidate crosses a final same-rule proof gate before it can affect an action."
>
  ...
  <Node id="proof" kind="decision">Complete same-rule proof?</Node>
  <Node id="visible" kind="decision">Visible detection-only finding</Node>
  ...
  <Edge from="finding" to="proof" />
  <Edge from="proof" to="action" label="complete exact proof" />
  <Edge from="proof" to="visible" label="raw, partial, or unpinned" />
  <Edge from="visible" to="action" label="no block authority" />
...
A fallback pattern preserves visibility but does not itself mint trusted-action
authority. Enforcement requires a complete code-owned proof pinned to the same
rule ID at the final gate.
...
but a raw pattern match remains detection-only at the trusted-action boundary.
