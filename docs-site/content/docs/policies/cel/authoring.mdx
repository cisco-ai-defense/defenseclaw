...
<Callout type="warn" title="A pattern preserves detection, not authority">
`pattern` and `tool_call_only` control where compatible regex detection runs;
they do not authorize a trusted-action block. A custom CEL rule may enforce
only when complete authoritative ActionFacts match its expression and the
final code-owned proof is pinned to that same rule ID. Raw regex, partial or
invalid projection, parser-shadow, and mismatched proof remain visible as
detection-only evidence.
</Callout>
