...
CodeGuard result, or a bounded exact fallback result. Raw regex matches,
parser-shadow evidence, partial or invalid projections, and unpinned results
remain visible as detection-only findings. The gate never removes a finding;
it only prevents incomplete evidence from authorizing a block.
