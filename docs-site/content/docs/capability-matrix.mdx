...
<Cards>
  <Card title="Family" description="proxy = DefenseClaw sits in the LLM data path. hooks = DefenseClaw hooks into the agent's lifecycle. surface = local inventory / asset support without runtime blocking." />
...
Microsoft Scout appears as `surface`: DefenseClaw can inventory Scout skills and install an opt-in CodeGuard skill asset, but it does not expose runtime block or HITL controls until Microsoft publishes a Scout hook or proxy contract.
...
Pick a row with **fail-closed: yes**: OpenClaw, ZeptoClaw, Claude Code, Codex, Cursor, Gemini CLI, GitHub Copilot CLI, OpenCode. For Hermes and Windsurf, configure operator-side timeouts.
