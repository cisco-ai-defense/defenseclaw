...
Narrow configured roots are honored in both modes. Select `passive` when an organization wants known stores and explicitly approved paths only; select `enhanced` when finding models owned by previously unknown applications is the priority.
...
- accepts high-signal GGUF/GGML, safetensors, Core ML, and Q4NX artifacts on format evidence, while ONNX/ORT, TFLite, PyTorch/checkpoint, and generic binary formats require a model/weights/checkpoint/format directory, a semantic model filename, or a nearby bounded metadata sidecar;
...
Speech, vision, embedding, and embedded models are inventory evidence, not discarded as noise. The macOS Models view filters the initial presentation rather than deleting those signals: by default it shows entries with at least 80% discovery confidence whose modality is `generative` or `unknown` and whose relevance is `primary` or `unknown`. A `model_api` row without model-specific `discovery_confidence` remains visible so an installed runtime model is not hidden merely because its gateway cannot classify it; the modality and relevance rules still apply. The **Model Filters** menu provides **Show All Models**, modality, and relevance controls for exposing the complete inventory. This keeps large application caches readable while preserving the broader evidence.
...
The full TUI `AI Discovery` panel groups local models case-insensitively by model ID in a separate compact table with state, model, country/flag, publisher, root, derivation, status, and format. Everything else remains in the existing agent/tool table. Press `t` to move keyboard focus between the product and model tables; arrow keys and Enter then operate on the visible selection. Model detail shows owner application, modality, relevance, discovery confidence, base models, quantization/distillation, recipe, device, `size_bytes`, pinned state, and provenance source/confidence.

The macOS app decodes the same wire shape and presents the same model/non-model split. Its inspector also shows owner application, modality, relevance, and discovery confidence. Under **Model Filters**, **Show All Models** removes the default confidence and classification exclusions. The modality picker offers Generative, Speech, Vision, Embedding, Audio, and Unknown; the relevance picker offers Primary, Supporting, Embedded, and Unknown. Selecting either classification explicitly removes both implicit recommended classification exclusions while retaining the 80% confidence threshold, so choosing Speech also reveals high-confidence supporting speech models. The separate Overview tile remains an **AI Agents** summary: it excludes `local_model` rows from its count and row cap so a large model inventory cannot hide agent rows.
