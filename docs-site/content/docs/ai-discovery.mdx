...
| **C** — agent actions | Kernel process, file, and identity events, gated on AI-agent process lineage | Endpoint Security, `cn_proc` + `fanotify`, ETW + Security log |
...
That gate is the primary false-positive control. These tactics are far too
ordinary on a developer machine to be worth reporting without an actor
attached:
...
```
  ✓ inference heartbeat: up via /proc
  ⚠ shadow egress: available but not running -- connection table unreadable
  ⚠ agent actions: unavailable -- eslogger not found; needs macOS 13+ and Full
    Disk Access granted to the process that launches the gateway
  processes: 412 read, 9 not fully readable | connections: 60 seen, 55 with no owner
  ⚠ 92% of connections could not be attributed to a process. Run the gateway
...
The local observability stack ships `defenseclaw-ai-runtime.json`, cross-linked
with the AI Discovery dashboard; its first row is coverage, before any finding.

### TUI

The `Runtime` panel (`N`) renders the same snapshot. The plane strip is always
