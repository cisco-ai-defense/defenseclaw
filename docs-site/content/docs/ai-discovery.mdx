...
| Knob | Purpose |
| --- | --- |
| `--scan-root` (repeatable) | Filesystem roots to walk. Defaults to `~`, `/Applications`, `/opt`. |
| `--scan-shell-history` / `--no-...` | Read `~/.bash_history`, `~/.zsh_history` for AI invocations. |
| `--scan-package-manifests` / `--no-...` | Look at `package.json`, `pyproject.toml`, `requirements.txt`. |
| `--scan-env` / `--no-...` | Enumerate process env *names* (values are never read). |
| `--scan-network-domains` / `--no-...` | Watch DNS for provider endpoints (Anthropic, OpenAI, Bedrock, …). |
| `--signature-pack` | Path or registry id of the signature pack. Default ships in the binary. |
| `--scan-interval` | Seconds between rescans. Default 300. |
...
```bash
defenseclaw agent confidence explain claude-code
...
<Steps>
  ...
  <Step>
    Approve specific entries as you go: `defenseclaw registry approve <id> --type skill`. Discovery + admission are now closed-loop.
  </Step>
</Steps>
...
<Accordions>
  ...
  <Accordion title="Discovery is too noisy on this host">
    Scope it: `defenseclaw agent discovery enable --scan-root ~/work --no-scan-shell-history --no-scan-network-domains`. The defaults are conservative for privacy, but on shared dev hosts you may also want to drop the package-manifest walk.
  </Accordion>
</Accordions>
