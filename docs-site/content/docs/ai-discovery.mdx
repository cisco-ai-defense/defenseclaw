...
inventory scanner's names-only rule. DNS capture and the host plane each need
...
| `--enable-host-plane` | Explicit opt-in for Plane C. Listing `c` in `planes` without this is ignored. |
