...
defenseclaw-gateway audit export --limit 100 --output - \
  | jq -r 'select(.connector) | .connector' | sort | uniq -c
```

`doctor` confirms connector health. `alerts` and the SQLite export histogram
show new events landing under the expected connector.
