...
| <ConnectorLabel id="hermes" /> | hook contract | `>=0.19.0, <0.21.0` then `>=0.21.0, <0.22.0` | `hermes-hooks-v1` / `hermes-hooks-v2` / `v6` | prompt, tool_call, tool_result, event_content |
...
| <ConnectorLabel id="opencode" /> | hook contract | `>=1.18.10, <1.19.0` (source-reviewed; current pin `1.18.31`) | `opencode-hooks-v1` / `v7` | tool_call, tool_result |
| <ConnectorLabel id="amp" /> | plugin contract | `>=0.0.1785334225` | `amp-plugin-v1` / `v2` | prompt, tool_call, tool_result, event_content |
| <ConnectorLabel id="omnigent" /> | hook contract | `>=0.7.0,<0.14.0` / pinned custom-policy API | `omnigent-custom-policy-v1` / `v1` | prompt, tool_call, tool_result |
| <ConnectorLabel id="kiro" /> | not gated | Kiro IDE and `kiro-cli --v3` read `.kiro/hooks/*.json` (workspace); `kiro-cli` 2.x reads its agent config | native hooks on both surfaces plus optional ACP contract lock | prompt, tool_call, tool_result; ACP adds permission, filesystem, terminal |
...
The active floors and bounds come from upstream release notes, current vendor docs, or source review: Codex `0.124.0` is the stable-hooks release. Cursor introduced beta hooks in `1.7.0`, but DefenseClaw's current 21-event contract is pinned exactly to Cursor Agent build `2026.07.23-e383d2b`; the application/Desktop `cursor_version` field is not accepted as Agent CLI version evidence. Hermes is bounded to `>=0.19.0,<0.21.0` (`hermes-hooks-v1`) and `>=0.21.0,<0.22.0` (`hermes-hooks-v2`), OpenHands starts at `1.12.0` and is source-reviewed against `1.16.0`, and Devin is pinned exactly to the source-reviewed `3000.4.25` CLI contract. Copilot CLI v1 starts at `1.0.18`, while v2 starts at `1.0.76`. OpenCode is bounded to the v1.18.10-v1.18.31 source-reviewed plugin/config/MCP contract. OmniGent is capped at `>=0.7.0,<0.14.0`. Antigravity (`agy`) starts at `1.1.8`; the protected harness currently pins official CLI `1.1.10`. Claude Code v1 is bounded to `>=2.1.154,<2.1.219`, and v2 begins at `2.1.219` with observational `DirectoryAdded`.
