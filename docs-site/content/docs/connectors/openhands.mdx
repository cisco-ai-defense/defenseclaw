...
DefenseClaw has no native Windows or WSL connector path for OpenHands. There
is no supported Git Bash, PowerShell, WSL, Docker, or mixed-path workaround;
run this connector on macOS or Linux instead.
</Callout>
