...
DefenseClaw v1 governs opencode tool execution via the bridge plugin only. opencode's MCP servers and provider config live in `opencode.json`, which DefenseClaw does not manage in v1, so MCP / skills / rules / plugins / agents surfaces are treated as unsupported and their panels are hidden while `claw.mode=opencode`.
