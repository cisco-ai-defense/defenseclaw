...
On Windows this is **supported with native-degraded limitations** for the reviewed OmniGent
0.7.x range (`>=0.7.0,<0.8.0`).
...
Official OmniGent agent YAML can declare local functions, MCP tools, and
subagents. DefenseClaw v1 neither modifies nor scans those sources, so MCP,
function, subagent, skill, rule, plugin, and agent-bundle inventory remains
unsupported and unverified instead of appearing as an empty live inventory.
