...
Native logs, metrics, and traces use three Codex-scoped loopback endpoints under `/otlp/codex/<scoped-token>/v1/...`. Setup generates the OTLP credential in `~/.defenseclaw/hooks/.otlp-codex.token` with owner-only permissions. It is separate from both the gateway master token and the Codex hook credential, is accepted only for the Codex OTLP namespace over loopback, and is redacted from gateway route telemetry. `config.toml` contains no gateway or hook credential in an exporter header. Teardown removes the credential only after the managed Codex config is restored successfully; a later setup creates a new credential.
...
<Files>
  ...
  <Folder name="~/.defenseclaw" defaultOpen>
    <Folder name="hooks">
      ...
      <File name=".otlp-codex.token (owner-only scoped OTLP credential)" />
    </Folder>
  </Folder>
</Files>
