...
<Video
  src="antigravity"
  caption="agy tries to write a Stripe secret to .env → PreToolUse hook matches SEC-STRIPE + PATH-ENV-FILE → tool call denied on every retry, even operator-approved ones → secret never reaches disk."
/>
