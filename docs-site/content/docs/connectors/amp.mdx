...
| --- | --- | --- |
| macOS and Linux | **Supported** | DefenseClaw installs Amp's user-scoped system policy plugin under `~/.config/amp/plugins`. |
| Native Windows x64 | **Supported** | Packaged Setup installs the same plugin contract under `%USERPROFILE%\.config\amp\plugins`, protects it with a user-only ACL, and exercises it in the deterministic Windows release gate. |
...
defenseclaw setup amp --workspace 'C:\path\to\repo'
defenseclaw setup amp --workspace 'C:\path\to\repo' --mode action
```
