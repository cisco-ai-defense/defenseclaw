...
| --- | --- | --- | --- | --- |
| `permissive` | c2, cognitive, commands, enterprise-data, local-patterns, secrets, sensitive-paths, trust-exploit | injection, pii, tool-injection (higher judge thresholds) | broadest; additionally suppresses all IP findings and selected file-inspection PII | same six sensitive tool definitions as the other packs |
...
- **The built-in local stores still write.** `audit_sinks: []` means no external audit-event destination. SQLite `audit_events` powers `defenseclaw alerts`, the TUI, and audit export; `~/.defenseclaw/gateway.jsonl` remains the live structured runtime log. Wire external sinks via [`setup splunk`](/docs/observability/splunk) or [`setup local-observability`](/docs/observability/local-observability).
