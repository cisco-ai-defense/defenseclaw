...
| --- | --- | --- | --- | --- | --- | --- |
| `DEFENSECLAW_ALLOW_CGNAT` | low | `unset` | — | When set, the SSRF guard accepts CGNAT (100.64.0.0/10) destinations; default is to refuse. | — | `cli/defenseclaw/registries/ssrf.py:41` — Python SSRF guard reads this var to whitelist CGNAT 100.64/10 destinations |
| `DEFENSECLAW_ALLOW_HOOK_CONTRACT_DRIFT` | low | `unset` | — | Bypass hook-contract version check during connector setup; off by default. | — | `cli/defenseclaw/commands/cmd_setup.py:2517` — Setup flow reads this to bypass connector hook-contract version check |
| `DEFENSECLAW_ALLOW_LOCAL_MCP_TARGETS` | low | `unset` | — | When set, MCP scanner accepts loopback / private-IP target URLs. | — | `internal/scanner/mcp.go:54` — MCP scanner accepts loopback / private-IP target URLs when set |
...
| `DEFENSECLAW_REDACTION_DISABLE` | low | `unset` | — | Alias for DEFENSECLAW_DISABLE_REDACTION; turning off PII / credential redaction in sinks. | — | `internal/gateway/agent_hook.go:752` — Alias-style consumer noted in agent-hook redaction docs |
...
| `DEFENSECLAW_TRUSTED_PROXY_CIDRS` | low | `unset` | — | Comma-separated CIDRs the gateway will trust X-Forwarded-For from. | — | `internal/gateway/requestctx.go:310` — trustedProxyEnvVar pinning for X-Forwarded-For trust |
...
| `DEFENSECLAW_DATA_DIR` | low | `unset` | — | Override for the gateway's data directory. | — | `internal/daemon/daemon.go:44` — Daemon resolves the data directory from this env var (overrides config) |
...
| Env var | Impact | Default | Accepted values | Purpose | Security concern | Consumers |
| --- | --- | --- | --- | --- | --- | --- |
| `DEFENSECLAW_JUDGE_PERSIST_QUEUE_SIZE` | low | `unset` | — | Override for the in-memory persist queue size for judge events. | — | `internal/config/config.go:1185` — Override for in-memory judge persist queue size |
...
| `DEFENSECLAW_LOG` | low | `unset` | — | Set to a level (debug / info / warn / error) to override gateway log verbosity. | — | `cli/defenseclaw/main.py` — CLI entrypoint reads log-level override |
...
| `DEFENSECLAW_SANDBOX_FORCE_REGEX_CLEANUP` | low | `unset` | — | Force the sandbox cleanup path to use the regex pruner instead of structured detection. | — | `cli/defenseclaw/commands/cmd_setup_sandbox.py:1258` — Sandbox cleanup script branches on this var |
...
| `DEFENSECLAW_TUI_BACKEND` | low | `unset` | — | Override TUI backend selection (legacy 'go' vs new 'python textual'). | — | `cli/defenseclaw/commands/cmd_tui.py` — TUI backend selector reads this to pick legacy vs Textual |
| `DEFENSECLAW_TUI_SKIP_FIRST_RUN_PROMPT` | low | `unset` | — | Skip the first-run TUI onboarding prompt — used by automation and tests. | — | `cli/defenseclaw/tui/__init__.py:53` — Skip the first-run TUI onboarding prompt for automation/tests |
...
| `DEFENSECLAW_BAKED_HOOK_PATH` | low | `unset` | — | Hook script's compile-time absolute path. | — | `internal/gateway/connector/hooks/_hardening.sh:4` — Hook hardening preamble seeds this from the daemon launcher |
...
| `DEFENSECLAW_HOOK_PATH_TRUSTED` | low | `unset` | — | Marks the hook process as launched from the trusted DEFENSECLAW_BAKED_HOOK_PATH location. | — | `internal/gateway/connector/hooks/_hardening.sh:119` — Hardening cleanup unsets this internal flag |
...
| `DEFENSECLAW_TRACEPARENT` | low | `unset` | — | W3C traceparent set by hook scripts when forwarding the inbound trace to the gateway. | — | `internal/gateway/connector/codex.go:970` — Codex hook script forwards inbound W3C traceparent via this env var |
| `DEFENSECLAW_TRACESTATE` | low | `unset` | — | W3C tracestate set by hook scripts when forwarding the inbound trace to the gateway. | — | `internal/gateway/connector/codex.go:971` — Codex hook script forwards inbound W3C tracestate via this env var |
...
| `DEFENSECLAW_O11Y_DASHBOARD_NAME_PREFIX` | low | `unset` | — | Optional prefix prepended to provisioned Splunk O11y dashboard names. | — | `cli/defenseclaw/commands/cmd_setup_splunk_o11y_dashboards.py:137` — Splunk O11y dashboards installer reads this prefix |
...
| `DEFENSECLAW_SPLUNK_O11Y_DASHBOARDS_WORK_DIR` | low | `unset` | — | Working directory the Splunk O11y dashboards Terraform run uses for state. | — | `cli/defenseclaw/commands/cmd_setup_splunk_o11y_dashboards.py:113` — Splunk O11y dashboards Terraform working dir |
| `DEFENSECLAW_TERRAFORM_PLUGIN_DIR` | low | `unset` | — | Optional override for the Terraform plugin cache directory used by the dashboards installer. | — | `cli/defenseclaw/commands/cmd_setup_splunk_o11y_dashboards.py:92` — Terraform plugin cache dir override |

## Test fixtures (test-only)

| Env var | Impact | Default | Accepted values | Purpose | Security concern | Consumers |
| --- | --- | --- | --- | --- | --- | --- |
| `DEFENSECLAW_AGENT_TTY_BIN` | low | `unset` | — | Test-only override for the TTY-spawning helper used by agent-launcher tests. | — |  |
| `DEFENSECLAW_AGENT_TTY_TESTS` | low | `unset` | — | Gate flag for TTY-spawning tests in agent-launcher suite. | — |  |
| `DEFENSECLAW_E2E_BEDROCK_MODEL` | low | `unset` | — | E2E test fixture: Bedrock model id to exercise. | — |  |
| `DEFENSECLAW_E2E_BEDROCK_REGION` | low | `unset` | — | E2E test fixture: Bedrock region. | — |  |
| `DEFENSECLAW_E2E_INSTANCE` | low | `unset` | — | E2E test fixture: target instance label. | — |  |
| `DEFENSECLAW_E2E_KEEP` | low | `unset` | — | E2E test fixture: keep working dirs after a run for inspection. | — |  |
| `DEFENSECLAW_E2E_MODEL` | low | `unset` | — | E2E test fixture: model id to exercise across providers. | — |  |
| `DEFENSECLAW_E2E_PORT` | low | `unset` | — | E2E test fixture: gateway port override. | — |  |
| `DEFENSECLAW_E2E_PROMPT_STRATEGY` | low | `unset` | — | E2E test fixture: prompt strategy ('benign' \| 'malicious'). | — |  |
...
| `DEFENSECLAW_TEST_CUSTOM_KEY` | low | `unset` | — | Test fixture: arbitrary custom-key env var used by keys command tests. | — |  |
