...
| `DEFENSECLAW_GATEWAY_URL` | — | `unset` (e2e tier skipped) | `http(s) base URL e.g. http://127.0.0.1:18970`, `unset` | Base URL of a running gateway for the opt-in security-suite e2e tier (TestSecuritySuiteE2E). | — | `internal/gateway/security_suite_test.go:307` — TestSecuritySuiteE2E reads this to locate the gateway; skips when unset |
