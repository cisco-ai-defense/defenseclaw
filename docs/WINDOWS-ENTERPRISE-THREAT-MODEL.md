...
| W-02 | User replaces an executable, script, config, manifest, metadata, or ledger | Fixed local NTFS roots; no reparse points; trusted owner and protected DACL on every managed path itself; content hashes/signatures. The **ancestor** chain above the managed roots is advisory since AIFW-34262 — those directories belong to the Cisco Secure Client installer (AVC), which re-ACLs them on its own schedule, so an untrusted grant there emits a `managed_trust_ancestor_advisory` warning instead of failing a load, install, or rollback; `DEFENSECLAW_MANAGED_TRUST_STRICT_ANCESTORS=1` restores the fatal verdict | Write/delete/rename/ACL probes fail; verify catches byte or ACL drift; ancestor advisory and strict-pin tests in `internal/managed` |
