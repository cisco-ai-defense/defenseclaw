...
| W-27 | An old officially signed or custom unsigned agent avoids a newer managed-hook contract | Application control allows only approved signed clients at or above the minimum versions; the minimums are the floors of the current hook contracts published in `cli/defenseclaw/inventory/hook_contracts.json`, today Codex 0.133.0 and Claude 2.1.152, with the separate Codex launcher gate still required | Approved clients start; mandatory explicitly supplied old signed Codex/Claude and custom unsigned lookalikes fail process creation with an application-control denial |
