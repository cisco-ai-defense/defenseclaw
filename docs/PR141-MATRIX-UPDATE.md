...
| **Hook auto-wired into agent's config** | ✅ via `before_tool_call` plugin hook | ❌ **not wired**: `patchZeptoClawConfig` only touches `providers.*.api_base` and `safety.allow_private_endpoints`. Comment on `zeptoclaw.go:30-35` now accurately states proxy-side response-scan, not config-based hooks. | ⚠️ preview: `patchClaudeCodeHooks` registers the exact version-selected matrix in `~/.claude/settings.json`: 28 events through 2.1.218, or 29 for 2.1.219+ with observational `DirectoryAdded` | ⚠️ Codex doesn't have a settings-based hook system; script sits on disk but Codex never invokes it on its own |
