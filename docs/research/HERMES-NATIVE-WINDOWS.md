...
Latest upstream version examined: **Hermes Agent v0.21.3, tag `v2026.9.14`,
released 2026-09-14**, commit
...
| --- | --- | --- |
| Release | [Hermes Agent v0.21.3 / `v2026.9.14` release](https://github.com/NousResearch/hermes-agent/releases/tag/v2026.9.14) | Published 2026-09-14. The source recheck confirms the same 23 claimed shell-hook events at the latest official release; `VALID_HOOKS` grew with observer-only events that DefenseClaw does not claim. It does not certify DefenseClaw packaging or client behavior. |
...
| Hook schema and responses | [Hook documentation at `v2026.9.14`](https://github.com/NousResearch/hermes-agent/blob/v2026.9.14/website/docs/user-guide/features/hooks.md) | `hooks.<event>[]` entries have `command`, optional matcher (honored only for pre/post tool), and timeout (60-second default, 300-second cap); JSON event on stdin; JSON response on stdout; `pre_tool_call` can block, `pre_llm_call` can add context, and `pre_verify` can continue the bounded verification loop. |
| Native process behavior | [`agent/shell_hooks.py` at `v2026.9.14`](https://github.com/NousResearch/hermes-agent/blob/v2026.9.14/agent/shell_hooks.py) | Command is expanded, split with `shlex.split`, and passed as argv to synchronous `subprocess.run` with `shell=False`, captured UTF-8 stdout/stderr, timeout, and Windows no-window flags. For `pre_tool_call`, exit code 2 blocks; other nonzero exits warn and open. Timeout can block only when that hook entry explicitly sets `fail_closed`; malformed output opens. |
| Hook/event inventory | [`hermes_cli/plugins.py` at `v2026.9.14`](https://github.com/NousResearch/hermes-agent/blob/v2026.9.14/hermes_cli/plugins.py) | `VALID_HOOKS` retains the same 23 events reviewed at v0.19.0 and adds observer-only events that DefenseClaw does not register. DefenseClaw classifies response authority independently rather than treating membership as enforcement. |
| Shell-hook approval | [`agent/shell_hooks.py` at `v2026.9.14`](https://github.com/NousResearch/hermes-agent/blob/v2026.9.14/agent/shell_hooks.py) | Consent remains persisted in `<home>/shell-hooks-allowlist.json` as exact `event` + `command` approvals. DefenseClaw preserves `hooks_auto_accept` and transactionally owns only its exact 23 approval pairs. |
| Callback lifecycle | [`agent/shell_hooks.py` at `v2026.9.14`](https://github.com/NousResearch/hermes-agent/blob/v2026.9.14/agent/shell_hooks.py) and [hook documentation at `v2026.9.14`](https://github.com/NousResearch/hermes-agent/blob/v2026.9.14/website/docs/user-guide/features/hooks.md) | Shell callbacks are registered into a running process. On-disk registration or revocation therefore requires reloading/restarting every affected Hermes host before it is live. No vendor-proven in-process reload command is claimed. |
...
and resume the tool call. In Hermes 0.21.x, exit code 2 is an enforcement
surface for `pre_tool_call`; other nonzero exits remain warnings. Missing
home/token, oversized stdin, gateway auth/network failure, gateway 4xx/5xx,
and malformed/missing response fields fail open without synthesizing block
JSON. Timeouts open unless the hook entry explicitly sets `fail_closed`.
DefenseClaw also passes through a valid gateway response containing a valid
Hermes block object.
...
| --- | --- | --- | --- | --- |
| 1 | Go/Python registry, discovery, version, platform mirrors | **Implemented** — Go/Python registries, `hermes --version`, `HERMES_HOME`/LocalAppData paths, `hermes-hooks-v1 >=0.19.0,<0.21.0`, `hermes-hooks-v2 >=0.21.0,<0.22.0`, and mirrored Windows `supported` status. Installed v0.17.0 is detected but unsupported, produces one observe downgrade with upgrade guidance, and is never accepted for the 23-event contract. Official v0.21.3 source keeps the claimed 23-event set and adds observer-only VALID_HOOKS that DefenseClaw does not claim; packaged and authentic-client validation metadata remain pending. | **Implemented** — supported native-Windows registry and versioned hook contract. | **Implemented** — supported native-Windows registry and versioned hook contracts. |
...
| 5 | Hook/policy version, events, matcher, wire, wait/timeout, modes | **Implemented** — the same 23 claimed events on v1 (0.19-0.20) and v2 (0.21.x), flat YAML entry shape, JSON stdin/stdout, synchronous wait, and 30-second DefenseClaw registration. **Officially limited:** block only `pre_tool_call`; context at `pre_llm_call`; bounded continue at `pre_verify`; transform/gateway/approval/API/Kanban/lifecycle behavior classified independently; no DefenseClaw ask. v1 fails open on non-zero exits; v2 blocks `pre_tool_call` on exit 2, and timeouts still fail open unless the hook spec sets `fail_closed`. | **Implemented** — connector-specific documented matrix; ask/block/fail-mode varies by event. | **Implemented** — connector-specific documented matrix; no native ask. |
...
| Fail-mode provenance | **Officially limited:** Hermes 0.21.x blocks `pre_tool_call` on exit code 2 and can fail closed on a timeout only when the hook entry explicitly requests it; other execution errors warn and open. DefenseClaw records the versioned configured/effective provenance and does not overstate broader fail-closed coverage. |
...
4. installed the official Hermes `v2026.9.14` client natively, without
