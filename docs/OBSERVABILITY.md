...
runtime does not infer an outbound exporter from `OTEL_EXPORTER_OTLP_*`.
...
separately. The gateway immediately exports one canonical `invoke_agent` root
for the connector session and parents every later `chat` and `execute_tool`
span to that same trace. This avoids disconnected one-span traces and
duplicate-looking tool rows in Galileo. Connectors with a post-model hook
export at that event; Codex and Claude Code use Stop as their model-completion
fallback. The root and completed tools export during long-running turns, so an
agent does not need to stop before Galileo receives activity. Correlation
caches are bounded, duplicate completions are suppressed, and all content
continues to follow persistent-sink redaction.
...
| **Agent360** | `defenseclaw-agent-360` | Automatic runtime Agent Directory and one-click agent/tree drill-down: durable lifecycle/execution identity, descendants, turns, models, tools, websites, inputs/outputs, reported tokens/cost, security decisions, Tempo traces and node graph. |
