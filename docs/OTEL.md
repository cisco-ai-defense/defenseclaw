...
Outbound routes do not inherit `OTEL_EXPORTER_OTLP_*`. Configure each endpoint,
protocol, TLS policy, signal set, and `${ENV_REFERENCE}` header explicitly in
`otel.destinations[]`. Standard OTLP variables remain available to connector
processes that emit OTLP into DefenseClaw; they do not create a gateway export
route.
