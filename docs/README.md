...
implementation contracts, architecture and design records, contributor and
release procedures, package-local notes, generated schema references, and test
fixtures.
...
| Observability v8 | [`OBSERVABILITY.md`](OBSERVABILITY.md), [`design/observability-v8/`](design/observability-v8/) | `schemas/telemetry/v8/`, `schemas/config/v8/`, `internal/observability/` |
...
Files under [`archive/`](archive/), [`design/`](design/),
[`development/`](development/), [`requirements/`](requirements/), and
[`specs/`](specs/) preserve decision context or executable design contracts.
Each file states its status. Historical claims describe the recorded change,
not current operator guidance; current behavior is determined by code, schema,
tests, and the published website.

The v7 observability documents
[`OBSERVABILITY-CONTRACT.md`](OBSERVABILITY-CONTRACT.md),
[`OTEL.md`](OTEL.md), and
[`OTEL-IMPLEMENTATION-STATUS.md`](OTEL-IMPLEMENTATION-STATUS.md) are retained
only for migration compatibility and archaeology. They are not v8 configuration
or telemetry authorities.
