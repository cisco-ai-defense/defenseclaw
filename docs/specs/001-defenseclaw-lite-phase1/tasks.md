# Tasks: DefenseClaw Edge Connector — Phase 1 (STANDARD Profile)
...
2. [x] **Platform HAL (Linux)** — Implement `hal_linux.c`: flash read/write (mmap'd
   file), monotonic tick source (clock_gettime CLOCK_MONOTONIC), secure storage
   paths, Unix socket creation. Maps to: REQ-12.

3. [x] **Core data structures** — Implement all static-allocation structs from
...
4. [x] **IPC hook + input validation** — Implement `ipc_hook.c`: Unix socket server,
...
5. [x] **Policy table engine** — Implement `policy_table.c`: binary search over
...
6. [x] **Unit tests: input validation + policy table** — Test cases for all
...
7. [x] **MQTT client** — Implement `mqtt_client.c`: MQTT 5.0 connect/subscribe/
...
8. [x] **CBOR codec** — Implement `cbor_codec.c`: encode/decode for heartbeat (32B),
...
9. [x] **Heartbeat + clock sync** — Implement heartbeat construction (32B format
   per S7.2), 30s timer loop, flag byte assembly. On verdict response receipt,
   update dclaw_clock_t from server_ts. Implement time_trusted logic.
   Maps to: REQ-24, REQ-25, REQ-26.

10. [x] **Verdict request/response protocol** — Implement verdict request publishing
    (CBOR encode, MQTT QoS 1) and response handling (CBOR decode, 16B format).
    Maps to: REQ-27.

11. [x] **Verdict HMAC verification + dedup** — Implement session_key derivation
...
12. [x] **Audit ring buffer** — Implement `audit_ring.c`: 256-entry ring in flash,
...
13. [x] **Capability correlator** — Implement `correlator.c`: 16-session FSM,
...
14. [x] **Integration test: device->MQTT->mock cloud->verdict** — Stand up mock MQTT
...
15. [x] **Fleet manager: device registry** — Go service (embedded in gateway):
    subscribe to `+/+/+/register` topic, parse registration CBOR, insert into
    iot_devices table, return MQTT response. Maps to: REQ-37.

16. [x] **Fleet manager: heartbeat processor** — Subscribe to `+/+/+/heartbeat`,
...
17. [x] **Fleet manager: anomaly detection + alerts** — Implement alert rules:
    device_offline (3x heartbeat interval), block_spike (5x baseline in 5min),
    audit chain break (HMAC mismatch). Dispatch via existing WebhookDispatcher.
    Maps to: REQ-39.

18. [x] **Verdict cache service** — In-memory hash map (Go map[string]VerdictEntry)
...
19. [x] **Policy compiler (Python)** — Implement `tools/policy_compiler.py`:
...
20. [x] **OTA receiver + A/B partition** — Implement `ota_receiver.c`: receive
...
21. [x] **Canary health-check window** — After policy apply, enter 10-min canary:
    track blocks/min vs. baseline (from policy blob or last-hour rate), auto-rollback
    to previous partition if spike 5x for 3 consecutive minutes. Report via heartbeat
    flag. Maps to: REQ-35.

22. [x] **Emergency broadcast handler** — Implement `dclaw_apply_emergency()`:
...
23. [x] **Integration test: full flow** — End-to-end: device boots, registers,
...
24. [x] **Speculative execution engine** — Implement escalation_mode lookup table,
...
25. [x] **Rate limiters** — Implement 3 token-bucket rate limiters configured from
    policy rate_limits section. Token refill on tick timer. Reject with
    DCLAW_REASON_RATE_LIMIT. Maps to: REQ-07.

26. [x] **AFL++ fuzz campaign** — Create fuzz harness targeting IPC parser
...
28. [x] **Prometheus metrics** — Expose fleet-level metrics from Go fleet manager:
...
29. [x] **Grafana dashboard + alert rules** — Create fleet dashboard JSON
...
30. [x] **Performance benchmark** — Measure on Raspberry Pi 4:
    - Local policy table decision latency (target: <5us)
    - Verdict cache hit latency (target: <10us)
...
31. [x] **Binary size + RAM audit** — Measure .text+.rodata from ELF (arm-none-eabi-size
    or equivalent). Generate linker map, sum all .bss + static allocations. Verify
    <80KB binary, <25KB RAM. Maps to: REQ-45, REQ-46, AC-12.

32. [x] **Acceptance test suite** — Automated tests for all 12 acceptance criteria
    (AC-01 through AC-12). Run on Raspberry Pi 4 hardware or QEMU aarch64 emulation.
    Maps to: All ACs.

33. [x] **Fleet REST API** — Implement all fleet management endpoints in Go
...
34. [x] **Documentation** — Operator guide (fleet setup, policy compiler usage,
...
35. [x] **Spec updates + CONTEXT.md** — Update architecture proposal version note,
    mark Phase 1 tasks complete, append CONTEXT.md entry.

### Content Scanner (Week 9-10)

36. [ ] **DFA pattern table data structures** — Add dfa_table_t (128 states x 256
    transitions), scan_context_t (6 DFA cursors + 4 findings), finding_t
    (category + severity + offset). Static allocation, zero heap.
    Maps to: REQ-53, REQ-54, REQ-55, REQ-56, REQ-57, REQ-58.

37. [ ] **DFA table walker** — Single-pass streaming matcher in content_scanner.c.
    Walks content buffer through all 6 category DFAs simultaneously. Returns
    findings array.
    Maps to: REQ-53, REQ-54, REQ-55, REQ-56, REQ-57, REQ-58.

38. [ ] **Secret detection patterns** — Compile DFA tables for: API key assignment,
    bearer token, AWS key, password assignment, token assignment. Port from
    gateway's secretPatternDetectors.
    Maps to: REQ-53.

39. [ ] **PII detection patterns** — Compile DFA tables for: SSN (XXX-XX-XXXX),
    credit card (Visa/MC/Amex/Discover). Port from gateway's
    defaultPIIDataRegexSources.
    Maps to: REQ-54.

40. [ ] **Credential prefix detection** — Compile DFA tables for: sk-*, AKIA*,
    ghp_*, xoxb-*, eyJ with length floors. Port from gateway's
    redaction/credentials.go.
    Maps to: REQ-55.

41. [ ] **Exfil intent detection** — Three-way conjunction DFA: sensitive target +
    read verb + egress verb within 240-byte window. Port from gateway's
    findStrongExfilIntent.
    Maps to: REQ-56.

42. [ ] **Injection/command detection** — DFA tables for shell metacharacters
    (;, &&, |, `, $()) and dangerous commands (rm -rf, chmod 777, curl|sh). Port
    from gateway's actionfacts/.
    Maps to: REQ-57, REQ-58.

43. [ ] **SSRF/netguard validation** — Add IP class checks to destination stage:
    block loopback, link-local, metadata, RFC1918. Inline credential rejection.
    Scheme validation. Private IP allowlist. Port from gateway's
    internal/netguard/.
    Maps to: REQ-59, REQ-60, REQ-61, REQ-62.

44. [ ] **Trust boundary inference** — Extend correlator: track content_scope per
    session (system/user_input/tool_output). First call = system, tool_result
    direction = tool_output, rest = user_input. Stricter rules on user_input.
    Maps to: REQ-63, REQ-64, REQ-65.

45. [ ] **Policy compiler DFA generation** — Extend policy_compiler.py: accept
    pattern strings in YAML, compile to DFA state machine tables, emit as C
    arrays in policy_tables.h and binary blob. Size validation for DFA tables.
    Maps to: REQ-53, REQ-54, REQ-55, REQ-56, REQ-57, REQ-58.

46. [ ] **Content scanner unit tests** — Test all 6 categories against known-good
    and known-bad inputs. Zero false negatives on test corpus.
    Maps to: AC-13.

### Enriched Escalation + Response (Week 11-12)

47. [ ] **IPC schema extension** — Add "direction" (request/response) and "content"
    (optional string) fields to JSON-RPC schema. Backward compatible: missing
    fields = Phase 1 behavior.
    Maps to: REQ-71, REQ-72, REQ-73.

48. [ ] **CBOR payload enrichment** — Extend verdict request encoding: add truncated
    content (max_escalation_payload_bytes configurable, default 1024), direction
    enum, content_scope, local findings array. Extend verdict response decoding:
    add category enum + 64-byte evidence.
    Maps to: REQ-66, REQ-67, REQ-68, REQ-69, REQ-70.

49. [ ] **Verdict cache enrichment** — Store category + evidence alongside action in
    cache entries. Return enriched verdicts on cache hits.
    Maps to: REQ-70.

50. [ ] **Integration + acceptance tests** — End-to-end: tool call with content ->
    content scan -> enriched escalation -> cloud verdict with category -> cached.
    Verify binary <80KB, RAM <25KB. AFL++ fuzz on content scanner.
    Maps to: AC-13, AC-14, AC-15, AC-16, AC-17.
...
| `content_scanner.c` | 100% branch | All 6 DFA categories, known-good/known-bad corpus, streaming partial buffers, zero-length input |
...
| Speculative execution | Device + slow cloud | PENDING returned <10us; retroactive BLOCK callback fires |
| Content scan + escalation | Device + MQTT + Fleet Manager | Tool call with secret in content -> local finding -> enriched escalation -> cloud verdict with category |
| Trust boundary enforcement | Device + mock LLM host | system scope call allowed; user_input scope with injection blocked; tool_output with exfil blocked |

### Performance Tests

| Test | Target | Method |
|------|--------|--------|
| Local decision latency | <5us P99 | 1M iterations, measure rdtsc/clock_gettime |
| Verdict cache hit | <10us P99 | 100K iterations with warm cache |
...
| Content scan throughput | <50us per 1KB buffer | 100K iterations through all 6 DFAs, measure wall time |
...
| `dclaw_content_scan()` | AFL++ | 1M minimum | Random bytes up to 4KB through all 6 DFA categories |
