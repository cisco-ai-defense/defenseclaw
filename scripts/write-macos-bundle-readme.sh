...
| \`lib/scrub_agent_configs.py\`    | Whole-file managed plugin scrubber for \`amp\` and \`opencode\` (stdlib Python) |
...
- macOS with \`launchctl\` (base image; no Xcode Command Line Tools
  ...
  binary).
- Python 3 (\`/usr/bin/python3\`) is required when
  \`sudo ./uninstall.sh --purge\` cleans an \`amp\` or \`opencode\`
  managed plugin. The whole-file scrubber
  (\`lib/scrub_agent_configs.py\`) restores or removes the plugin only
  under its private connector- and target-bound receipt authority and
  uses Python's stdlib SHA-256 implementation for exact verification.
  Codex, Claude Code, and Cursor scrub through the Go binary and do not
  need Python.
