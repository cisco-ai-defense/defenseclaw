...
# Continuation driver. s2-C0 already settled by the previous driver.
# Reordered so the cheap high-value arms land before the multi-hour s3 stages.

# 1. s2 bare-command serialisation -- SecJudge's best case (~55 tokens/decision)
...
# 3. s3 bare-command serialisation
...
# 4. s2 C7 -- the exact parity context (41% truncation)
...
# 5. toolcall-labels -- CONTAMINATED LANE (>=32 byte-identical long attack payloads shared with
#    the DefenseClaw Security Suite, a 5x-weighted SecJudge training source; benign half is a
#    DefenseClaw dump of unmeasurable overlap). Run last and label as contaminated everywhere.
...
# 6. s3 C7 -- 28.5M padded tokens, last on purpose
