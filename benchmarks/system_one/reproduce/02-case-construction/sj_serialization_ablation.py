...
for name, fn in SERIALISATIONS.items():
    ...
    def run_batch(batch):
        if not batch:
            return
        enc = tok([t for _, t in batch], truncation=True, max_length=512, padding=True, return_tensors="pt")
        out = model(input_ids=enc["input_ids"], attention_mask=enc["attention_mask"])
        for i, (cid, _t) in enumerate(batch):
...
Path("$WORK/.system-one-data/outputs/secjudge/serialisation-ablation.json").write_text(
