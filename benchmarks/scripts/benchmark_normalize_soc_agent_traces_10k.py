...
"""Normalize pinned SOC-Agent-Traces-10K investigation calls conservatively.
...
Only a closed set of exact investigation schemas from an explicitly successful
source trace is scored benign. Unknown, failed, mutating, administrative,
shell, remediation, response, secret-bearing, and diagnosis-bearing calls
remain contextual or are excluded when they cannot be safely projected.

This is deliberately a separate adapter from the companion 100K corpus. Its
three shards, source schema, fingerprints, namespaces, and authority contract
are independently pinned; a 100K shard can never satisfy this adapter.
"""
...
SCHEMA_VERSION = "1"
DATASET_ID = "soc-agent-traces-10k"
SOURCE_URL = "https://huggingface.co/datasets/alirezaaminzadeh/soc-agent-traces-10k"
SOURCE_REVISION = "d69237c8890dea0f34400a33af488bc5ac108e83"
...
SOURCE_FILES = {
    "development": (
        ...
        21_317_565,
        "3f9c6c73ee727bf715bc4394dd47205abffe2f86063c79ff233053f3292961c5",
    ),
    "validation": (
        "data/validation-00000-of-00001.parquet",
        1_172_120,
        "d34a897a80b6a10d35110c46f5cd9b47125d3807e45aeb6bd9b413c97c6fcdb6",
    ),
    "test": (
        "data/test-00000-of-00001.parquet",
        1_267_420,
        "5ed4f3ce90546bc079b88df0113a16e5c50c0f9a4fee67c10580438beb64d366",
    ),
}
ADAPTER = "soc-agent-traces-10k-read-only-v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-root", type=Path, required=True, help="Pinned SOC-Agent-Traces-10K root")
...
def redaction_token(kind: str, value: str) -> str:
    return f"REDACTED_{kind}_{digest('soc-agent-10k-redaction-v1', kind, value)[:16]}"
...
def truth(benign: bool, reason: str) -> dict[str, Any]:
    if benign:
        return {
            ...
            "label_source": "adapter:soc-agent-10k-explicit-success-read-only-v1",
    ...
    return {
        ...
        "label_source": "adapter:soc-agent-10k-contextual-contract-v1",
...
def case_base(
    *,
    ...
    return {
        ...
        "id": f"soc-agent-10k/{trajectory[:16]}/{suffix}",
        "source": {
            "dataset": DATASET_ID,
            "revision": revision,
...
def normalize_row(
    ...
    explicit_success = row.get("success") is True
    statistics["source_traces_explicit_success" if explicit_success else "source_traces_not_explicit_success"] += 1
    trajectory = digest("soc-agent-10k-trajectory-v1", revision, scenario_id)[:24]
    split_group = digest("soc-agent-10k-split-group-v1", revision, scenario_id)[:24]
    ...
    for call in calls:
        ...
        if (
            ...
            events.append(None)
            continue
        schema_is_read_only = read_only_schema(call.tool_name, call.arguments)
        benign = explicit_success and schema_is_read_only
        if benign:
            reason = "benign"
        elif not explicit_success:
            reason = "source_trace_not_explicitly_successful"
        else:
            reason = "tool_or_arguments_not_in_closed_read_only_contract"
        ...
        candidates.append(
            Candidate(
                ...
                digest("soc-agent-10k-payload-v1", "action", canonical_json(payload)),
                case,
            )
        )
        ...
        candidates.append(
            Candidate(
                ...
                digest("soc-agent-10k-payload-v1", "stateful", canonical_json(stateful_payload)),
                stateful,
            )
        )
...
def deduplicate(candidates: Sequence[Candidate], statistics: Counter[str]) -> list[dict[str, Any]]:
    ...
    for values in grouped.values():
        if len({item.case["split"] for item in values}) > 1:
            statistics["exact_payload_cross_split_excluded"] += len(values)
            continue
        contracts = {
            (
...
def normalize_records(
    ...
    *,
    ...
    for split, row in records:
        ...
        try:
            projected, row_statistics = normalize_row(row, revision=revision, split=split)
        except ProjectionError as exc:
...
def normalize_rows(
    ...
    *,
    ...
    """Normalize in-memory fixtures without weakening production fingerprints."""
    return normalize_records(((split, row) for row in rows), revision=revision)
...
def parquet_rows(path: Path) -> Iterable[Mapping[str, Any]]:
    ...
    required = {"scenario_id", "success", "trace"}
...
def normalize_source_root(
    ...
    *,
    ...
    for split, (relative_path, expected_bytes, expected_sha256) in SOURCE_FILES.items():
        candidate = resolved_root / relative_path
        if candidate.is_symlink():
            raise ValueError(f"invalid source shard: {relative_path}")
        path = candidate.resolve(strict=True)
        try:
            path.relative_to(resolved_root)
        except ValueError as exc:
            raise ValueError(f"source shard escapes source root: {relative_path}") from exc
        if not path.is_file() or path.stat().st_size > MAX_FILE_BYTES:
            raise ValueError(f"invalid or oversized source shard: {relative_path}")
        source_bytes = path.stat().st_size
        source_sha256 = file_sha256(path)
        if verify_pinned_file and (source_bytes != expected_bytes or source_sha256 != expected_sha256):
            raise ValueError(f"pinned SOC-Agent-Traces-10K shard identity mismatch: {relative_path}")
        source_files.append(
            {
                "split": split,
                "path": relative_path,
        ...
        records.extend((split, row) for row in parquet_rows(path))
    return normalize_records(records, revision=revision, source_files=source_files)
...
def main() -> int:
    args = parse_args()
    cases, manifest = normalize_source_root(args.source_root, revision=args.revision)
