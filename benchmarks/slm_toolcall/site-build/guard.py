...
ALLOWED_EXT = {".html", ".css", ".md", ".json", ".txt"}
MAX_BYTES = 1024 * 1024
...
# Symmetric with the corpus-side abort above. Without this the guard prints
# "GUARD RESULT: PASS" after scanning nothing, which is what a wrong SITE path
# looks like — and the default SITE contains an unexpanded "$WORK", so a bare
# `python3 guard.py` walks a directory that does not exist and passes on zero
# files. A vacuous pass on the payload is worse than one on the corpora,
# because the payload is the thing being published.
if not files:
    print(f"ABORT: no payload files found under {SITE}, so the guard would be vacuous.")
    print("       Pass the built site directory as the first argument.")
    raise SystemExit(2)
