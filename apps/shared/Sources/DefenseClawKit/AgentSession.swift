...
public final class AgentSession: @unchecked Sendable {
    ...
    /// Send a user chat message as a raw gateway chat frame.
    ///
    /// **Protocol** (from OpenClaw gateway / BlueClaw reference):
    /// ```json
    /// { "type": "chat", "payload": {
    ///     "role": "user",
    ///     "content": [{ "type": "text", "id": "<uuid>", "text": "<user-text>" }],
    ///     "timestamp": "<iso8601>"
    /// }}
    /// ```
    /// Response comes as streaming gateway events.
    public func sendMessage(_ text: String) {
        ...
        Task {
            do {
                log.info("gateway", "Sending chat frame", details: "sessionKey=\(activeSessionKey ?? "nil") msg=\(text.prefix(80))")
                try await sendChatFrameAsync(text: text)
                log.info("gateway", "Chat frame sent")
            } catch {
                log.error("gateway", "Chat frame send failed", details: "\(error)")
                await MainActor.run {
    ...
    private func sendChatFrameAsync(text: String) async throws {
        guard let webSocket else { throw AgentSessionError.notConnected }
    ...
    static func isDangerousCommand(_ cmd: String) -> Bool {
        ...
        let patterns = [
            ...
            "python " + "-c", "perl " + "-e", "ruby " + "-e", "rm " + "-rf /",
            "dd " + "if=", "mk" + "fs", "chmod " + "777",
            "> /" + "etc/", ">> /" + "etc/", "passwd", "shadow", "sudoers"
        ]
