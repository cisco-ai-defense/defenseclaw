...
def test_macos_contract_smoke_may_explicitly_skip_missing_claude_selection(
    tmp_path,
    monkeypatch,
) -> None:
    ...
    with (
        ...
        patch(
            ...
            return_value=({}, {"claudecode": "agent is intentionally absent"}),
        ),
    ):
...
def test_macos_production_setup_requires_claude_selection(tmp_path, monkeypatch) -> None:
    ...
    with (
        ...
        patch(
            "defenseclaw.agent_selection.record_setup_agent_selections",
            return_value=({}, {"claudecode": "agent is absent"}),
        ),
        pytest.raises(Exception, match="freshly verified selected agent executable"),
    ):
        _record_windows_setup_agent_selections(tmp_path, ("claudecode",))
