...
def test_post_hard_cut_candidate_limits_direct_sources_without_auto_bridge(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    policy = tmp_path / "upgrade-baselines.json"
    policy.write_text(
        json.dumps(
            {
                "schema_version": 2,
                "published_baselines": ["0.8.6", "0.8.5", "0.8.4", "0.8.3"],
                "published_baseline_config_versions": {
                    "0.8.6": 8,
                    "0.8.5": 8,
                    "0.8.4": 7,
                ...
                "platform_published_baselines": {"windows": ["0.8.6", "0.8.3"]},
            }
        ),
        encoding="utf-8",
    )
        ...
        json.dumps(
            {
                ...
                "required_bridge_version": "0.8.4",
                "auto_bridge_from": [],
                "tested_source_versions": ["0.8.6", "0.8.5"],
                "platform_tested_source_versions": {"windows": ["0.8.6"]},
                "release_artifacts": release_candidate._expected_release_artifacts("0.8.7"),
            }
        ),
        encoding="utf-8",
    )

    release_candidate._validate_upgrade_manifest(manifest_path, "0.8.7")
