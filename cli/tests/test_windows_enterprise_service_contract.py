...
def test_certification_threads_broker_and_vendor_provider_through_lifecycle() -> None:
    ...
    # The sensor helper is unrelated to CMID and stays a required source on
    # main, so the requirements message lists it alongside the other three.
    # It names exactly what the condition tests.
    assert (
        "Upgrade requires -BrokerBinary, -GatewayBinary, -HookBinary, "
        "and -SensorHelperBinary" in module
    )
...
def test_legacy_service_recovery_restores_only_authenticated_preimage() -> None:
    ...
    # Upstream (release-26.8.4) spells this `$gatewayDependency` and switches
    # it wholesale, because the release branch has no sensor helper. On main
    # the gateway also depends on the privileged sensor helper, which is
    # independent of the Broker, so the two legs compose: the Broker leg drops
    # out on the rollback-only restore lane while a registered helper still
    # applies. Pin the composed form, and pin that the Broker leg is still what
    # the restore switch removes.
    assert (
        "$gatewayDependencies = if ($RestoreTransactionWithoutBroker) "
        "{ '' } else { $BrokerServiceName }" in configure
    )
    assert "if ($sensorHelperRegistered)" in configure
    assert "'{0}/{1}' -f $gatewayDependencies, $sensorHelperServiceName" in configure
    # sc.exe takes a bare '/' to mean "no dependencies"; an empty string would
    # be rejected, so the composed value must fall back to it.
    assert "$gatewayDependencies = '/'" in configure
    assert "'depend=', $gatewayDependencies," in configure
