...
def test_posix_resolver_bootstraps_recovery_under_fixed_mutator_lease() -> None:
    ...
    assert 'document.get("schema_version") != 3' in text
...
def test_posix_same_version_noop_reports_actual_provenance_before_mutation() -> None:
    ...
    contract = text.rindex("prepare_release_contract\nresolve_staged_upgrade")
...
def test_bridge_controller_hard_cut_is_resolver_only_before_mutation() -> None:
    ...
    journal_guard = main.index("if os.path.lexists(recovery_journal):")
