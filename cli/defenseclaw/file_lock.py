...
if os.name == "nt":
    ...
    def _lock_file_exclusive(file_obj: IO[str]) -> None:
        """Block until the sentinel byte at offset zero is locked."""
        while True:
            file_obj.seek(0)
            try:
                ...
                # LK_LOCK waits for a bounded period before raising. Retry so
                # the public primitive matches POSIX's blocking flock.
                time.sleep(0.05)
...
@contextmanager
def locked_file_update(path: str) -> Iterator[None]:
