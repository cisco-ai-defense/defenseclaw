...
def search(app: AppContext, query: str, as_json: bool) -> None:
    ...
    Delegates to ``npx clawhub search <query>``. This queries the remote
    ClawHub registry of installable skills — it does NOT list or search the
    skills already installed under a connector (use ``skill list`` for that).
