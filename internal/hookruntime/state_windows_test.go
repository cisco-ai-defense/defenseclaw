...
func TestStableRuntimePublishDisableAndReinstall(t *testing.T) {
	...
	if err := publishAt(paths, firstSource, firstDataRoot, stableRuntimeTransactionOne); err != nil {
	...
	if err := publishAt(paths, secondSource, secondDataRoot, stableRuntimeTransactionOne); err != nil {
	...
	if !samePath(reinstalled.DataRoot, secondDataRoot) || reinstalled.TransactionID != stableRuntimeTransactionOne {
...
func TestStableRuntimeFailsClosedForTamperedLauncherAndState(t *testing.T) {
	...
	if err := publishAt(paths, writeRuntimeSource(t, "MZ-trusted"), dataRoot, stableRuntimeTransactionOne); err != nil {
		t.Fatal(err)
	}
	...
	if err := publishAt(paths, writeRuntimeSource(t, "MZ-restored"), dataRoot, stableRuntimeTransactionTwo); err != nil {
		t.Fatal(err)
	}
...
func TestStableRuntimeMissingLauncherCannotReactivateDisabledState(t *testing.T) {
	...
	if err := publishAt(paths, writeRuntimeSource(t, "MZ-old"), filepath.Join(t.TempDir(), "data"), stableRuntimeTransactionOne); err != nil {
		t.Fatal(err)
	}
