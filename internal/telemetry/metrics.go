...
// normalizeMetricIdentityLabel keeps distinct long/opaque upstream IDs
// distinct without placing unbounded or unsafe text in Prometheus labels.
// Readable IDs remain readable; values that exceed the label contract or use
// unsupported characters become a stable 64-bit SHA-256 prefix.
func normalizeMetricIdentityLabel(value string) string {
	trimmed := strings.TrimSpace(value)
	if trimmed == "" {
		return "unknown"
	}
	normalized := NormalizeMetricTextLabel(trimmed)
	if len(trimmed) <= 64 && normalized == strings.ToLower(trimmed) {
		return normalized
	}
