...
func CaptureWindowsClaudeManagedPolicySnapshot(
	...
	err = windowsClaudeManagedPolicyTransaction(func() error {
		...
		// Reclaim any orphaned policy left by a partial prior uninstall
		// before we snapshot. Otherwise ownership validation below deadlocks
		// re-install on the "policy-without-state" asymmetry that a partial
		// teardown always produces. No-op unless the exact orphan signature
		// is observed on disk.
		if err := reclaimOrphanClaudeManagedPolicy(path, statePath); err != nil {
			return err
		}
...
func ReadWindowsClaudeManagedPolicyTargets() ([]string, bool, error) {
	...
	// A policy file without its DefenseClaw state sidecar is a forward
	// orphan in our declared namespace. Report it inactive so the later
	// authenticated mutation path can reclaim it. The reverse shape is
	// different: an existing state sidecar can authenticate exact
	// missing-policy recovery, so it must continue through strict validation
...
func installWindowsClaudeManagedPolicyUnlocked(body []byte, opts connector.SetupOpts, targetSID *windows.SID) (string, func() error, error) {
	...
	statePath := filepath.Join(filepath.Dir(path), windowsClaudeManagedStateFile)
	// Reclaim any orphaned policy left by a partial prior uninstall
	// before we snapshot. Otherwise ownership validation below refuses
	// this write forever on any machine whose previous teardown could
	// not fully remove the policy file. No-op unless the exact orphan
	// signature (policy file present, state sidecar absent) is on
	// disk. See reclaimOrphanClaudeManagedPolicy for the trust
	// argument.
	if err := reclaimOrphanClaudeManagedPolicy(path, statePath); err != nil {
		return "", nil, err
	}
...
// reclaimOrphanClaudeManagedPolicy deletes only a stale policy file whose
// ownership sidecar is absent. That forward orphan is in DefenseClaw's
// declared 90-defenseclaw.json namespace and cannot authenticate any prior
// state. A reverse orphan is deliberately not reclaimed here: its sidecar is
// the authority for validateWindowsClaudeMissingPolicyRecovery, so deleting
// it before validating schema, canonical bytes, policy digest, hook/service
// identity, and target enrollment would turn tampered state into a fresh
// install.
//
