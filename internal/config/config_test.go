...
func TestLoadRejectsFlatSignalsWithNamedDestinations(t *testing.T) {
...
`)
	...
	_, err := Load()
	if err == nil || !strings.Contains(err.Error(), "otel.traces.enabled") {
		t.Fatalf("Load error=%v, want mixed-shape field diagnostic", err)
	}
}
