...
func New(regoDir string) (*Engine, error) {
	...
	store, status, err := loadStoreWithStatus(regoDir)
	if err != nil {
		return nil, err
	}
	return &Engine{regoDir: regoDir, store: store, quarantineInvalid: true, status: EngineStatus{Generation: 1, AgentControl: status}}, nil
}
...
func (e *Engine) Reload() error {
	store, status, err := e.loadStoreWithStatus()
	if err != nil {
		return err
	}
...
func (e *Engine) loadStoreWithStatus() (storage.Store, AgentControlPolicyStatus, error) {
	if e.exactDataPath {
		store, err := loadStoreExact(e.regoDir)
		return store, AgentControlPolicyStatus{SchemaVersion: 1}, err
	}
	return loadStoreWithStatus(e.regoDir)
}
...
func loadStoreWithStatus(regoDir string) (storage.Store, AgentControlPolicyStatus, error) {
	...
	mergeSupplementalData(regoDir, data, "data-sandbox.json")
	status, err := loadAgentControlData(regoDir, data)
	if err != nil {
		return nil, AgentControlPolicyStatus{}, err
	}
	return inmem.NewFromObject(data), status, nil
}
