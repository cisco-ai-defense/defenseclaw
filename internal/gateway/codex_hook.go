...
func (a *APIServer) handleCodexHook(w http.ResponseWriter, r *http.Request) {
	...
	if a.otel != nil {
		a.otel.RecordCodexHook(r.Context(), req.HookEventName, resp.Action, resp.Severity, resp.Mode, resp.WouldBlock, elapsedMs)
		_ = a.otel.EmitCodexHookSpan(r.Context(), req.HookEventName, resp.Action, resp.Severity, resp.Mode, resp.WouldBlock, elapsedMs)
	}
	emitLifecycle(r.Context(), string(gatewaylog.SubsystemCodex), "completed", map[string]string{
	...
	if a.logger != nil {
		_ = a.logger.LogActionCtx(r.Context(), "codex-hook", req.HookEventName,
			fmt.Sprintf("action=%s raw_action=%s severity=%s mode=%s would_block=%v prompt_len=%d output_len=%d",
				resp.Action, resp.RawAction, resp.Severity, resp.Mode, resp.WouldBlock, len(req.Prompt), codexToolResponseLen(req.ToolResponse)))
	}
...
func (a *APIServer) scanCodexComponent(ctx context.Context, component, target string) bool {
	...
	if a.otel != nil {
		a.otel.RecordCodexComponentScan(ctx, component, status)
	}
	emitDiagnostic(ctx, "codex", "component scan", map[string]string{
		"component": component,
		"status":    status,
	})
	return err == nil
