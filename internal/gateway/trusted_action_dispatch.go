...
type trustedActionRequest struct {
	...
	// ASRContext is trusted caller context. A zero/nil context disables ASR
	// projection; dispatch never infers platform or provenance from command
	// text. The runtime and report hook are private test/replay seams.
	ASRContext            *actionfacts.ASRProjectionContext
	asrRuntime            actionSemanticsRuntime
	recordActionSemantics func(actionSemanticsBridgeReport)
...
type trustedActionTelemetry struct {
	...
	ASRProjectableCount                 int64
	ASRCandidateAuthoritativeCount      int64
	ASRCompleteCount                    int64
	ASRPartialCount                     int64
	ASRInvalidCount                     int64
	ASRUnsupportedCount                 int64
	ASRRuntimeErrorCount                int64
	ASRAuthoritativeCount               int64
}
...
func (t *trustedActionTelemetry) merge(other trustedActionTelemetry) {
	...
	t.ASRProjectableCount += other.ASRProjectableCount
	t.ASRCandidateAuthoritativeCount += other.ASRCandidateAuthoritativeCount
	t.ASRCompleteCount += other.ASRCompleteCount
	t.ASRPartialCount += other.ASRPartialCount
	t.ASRInvalidCount += other.ASRInvalidCount
	t.ASRUnsupportedCount += other.ASRUnsupportedCount
	t.ASRRuntimeErrorCount += other.ASRRuntimeErrorCount
	t.ASRAuthoritativeCount += other.ASRAuthoritativeCount
}
...
func (t trustedActionTelemetry) empty() bool {
	return t.ParserUncertaintyCount == 0 &&
		...
		t.ASRProjectableCount == 0 &&
		t.ASRCandidateAuthoritativeCount == 0 &&
		t.ASRCompleteCount == 0 &&
		t.ASRPartialCount == 0 &&
		t.ASRInvalidCount == 0 &&
		t.ASRUnsupportedCount == 0 &&
		t.ASRRuntimeErrorCount == 0 &&
		t.ASRAuthoritativeCount == 0
}
...
func dispatchTrustedAction(
	...
	facts = actionfacts.Analyze(request.Input)
	analyzed = true
	if request.ASRContext == nil {
		request.ASRContext = localActionSemanticsContext(request.Input)
	}
	if request.ASRContext != nil {
		...
		telemetry.merge(actionSemanticsTelemetry(report))
		if request.recordActionSemantics != nil {
			request.recordActionSemantics(report)
		}
	}

	// Shadow ASR loading/evaluation must not consume the authoritative rule
	// evaluator's bounded time budget. In particular, a cold embedded-runtime
	// load must not make the first action take an exact-fallback route while
	// identical warm actions take the semantic route.
