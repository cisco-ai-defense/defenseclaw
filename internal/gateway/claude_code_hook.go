...
func (a *APIServer) handleClaudeCodeHook(w http.ResponseWriter, r *http.Request) {
	...
	if a.otel != nil {
		a.otel.RecordClaudeCodeHook(r.Context(), req.HookEventName, resp.Action, resp.Severity, resp.Mode, resp.WouldBlock, elapsedMs)
		_ = a.otel.EmitClaudeCodeHookSpan(r.Context(), req.HookEventName, resp.Action, resp.Severity, resp.Mode, resp.WouldBlock, elapsedMs)
	}
	emitLifecycle(r.Context(), string(gatewaylog.SubsystemClaudeCode), "completed", map[string]string{
		"hook_event":  req.HookEventName,
		"action":      resp.Action,
		"raw_action":  resp.RawAction,
		"severity":    resp.Severity,
		"mode":        resp.Mode,
		"would_block": fmt.Sprintf("%v", resp.WouldBlock),
	})
	if a.logger != nil {
		_ = a.logger.LogActionCtx(r.Context(), "claude-code-hook", req.HookEventName,
			fmt.Sprintf("action=%s raw_action=%s severity=%s mode=%s would_block=%v prompt_len=%d output_len=%d",
				resp.Action, resp.RawAction, resp.Severity, resp.Mode, resp.WouldBlock, len(req.Prompt), claudeCodeOutputLen(req)))
	}
...
func (a *APIServer) scanClaudeCodeComponent(ctx context.Context, component, target string) bool {
	...
	if err != nil {
		status = "error"
	} else if result != nil && a.logger != nil {
		_ = a.logger.LogScanWithCorrelation(ctx, result, "", ScanCorrelationFromContext(ctx))
	}
	if a.otel != nil {
		a.otel.RecordClaudeCodeComponentScan(ctx, component, status)
	}
	emitDiagnostic(ctx, "claude-code", "component scan", map[string]string{
		"component": component,
		"status":    status,
	})
	return err == nil
