...
// hookSessionTrace is the live correlation anchor for one hook connector
// session. The root agent span is exported as soon as the first prompt (or
// first completed tool, when a prompt hook was missed) is observed. Later
// model and tool spans reuse its SpanContext, so they are exported in real
// time while still forming one Galileo trace instead of a set of unrelated
// one-span traces.
...
// exported agent root. Ending the root immediately is intentional: OTel SDKs
// do not export open spans, and hook sessions may run for hours. Children can
// therefore appear in Galileo as soon as they complete without waiting for a
// Stop/SessionEnd hook, while the retained SpanContext preserves hierarchy.
func (a *APIServer) ensureHookSessionTrace(
	...
	a.llmPromptMu.Lock()
	defer a.llmPromptMu.Unlock()
	if existing, ok := a.hookSessionTraces[key]; ok && existing.spanContext.IsValid() {
	...
	a.hookSessionTraces[key] = hookSessionTrace{spanContext: spanContext, meta: meta}
...
func (a *APIServer) emitHookLLMSpan(ctx context.Context, meta llmEventMeta, response string) {
	...
	parentCtx := a.ensureHookSessionTrace(ctx, llmEventMeta{
		Source: meta.Source, Provider: provider, SessionID: sessionID,
		AgentName: agentName, AgentType: agentType, AgentID: agentID,
	}, prompt)
