...
// rule generation is installed separately; this gate prevents local triage
// and bundled patterns from leaking into Agent Control-only mode.
...
func (g *GuardrailInspector) inspectRegexOnly(ctx context.Context, direction, content string, messages []ChatMessage, model, mode string) *ScanVerdict {
	...
	if g.agentControlRegexOnly() {
		findings := ScanAllRules(content, "")
		if len(findings) > 0 {
			ids := make([]string, 0, len(findings))
			for _, finding := range findings {
				ids = append(ids, finding.RuleID+":"+finding.Title)
			}
			localResult = &ScanVerdict{
				Action:         guardrailFallbackActionForSeverity(HighestSeverity(findings)),
				Severity:       HighestSeverity(findings),
				Reason:         "matched: " + strings.Join(ids, ", "),
				Findings:       ids,
				Scanner:        "agent-control",
				ScannerSources: []string{"agent-control"},
			}
		} else {
			localResult = allowVerdict("agent-control")
		}
	} else {
	...
	if sm == "local" || (localResult != nil && localResult.Severity == "HIGH" && !g.managedMode) {
		if localResult != nil {
			if g.agentControlRegexOnly() {
				localResult.Scanner = "agent-control"
...
func (g *GuardrailInspector) inspectRegexJudge(ctx context.Context, direction, content string, messages []ChatMessage, model, mode string) *ScanVerdict {
	...
	if len(ruleFindings) > 0 {
		...
		if g.agentControlRegexOnly() {
			ruleVerdict.Scanner = "agent-control"
			ruleVerdict.ScannerSources = []string{"agent-control"}
		}
	}
