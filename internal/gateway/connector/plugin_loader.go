...
// validatePluginOwner. Same trade-off as validatePluginPermissionsFromInfo:
// avoids an extra Lstat after the fd-bound check.
...
func validatePluginRootChain(dirPath string) error {
	...
	for i := 0; i < 1024; i++ {
		...
		if stat.Uid != gatewayUID && stat.Uid != 0 {
			return fmt.Errorf("ancestor %s owner uid=%d is neither the gateway uid=%d nor root", cur, stat.Uid, gatewayUID)
		}
...
func safeOpenPluginSO(soPath string) (*os.File, os.FileInfo, error) {
	...
	if runtime.GOOS != "windows" {
		lStat, lOK := lstatInfo.Sys().(*syscall.Stat_t)
		fStat, fOK := fdInfo.Sys().(*syscall.Stat_t)
		if !lOK || !fOK {
			f.Close()
			return nil, nil, fmt.Errorf("%s: could not compare lstat/fstat inode (non-unix FS?)", soPath)
		}
		if lStat.Ino != fStat.Ino || lStat.Dev != fStat.Dev {
			f.Close()
			return nil, nil, fmt.Errorf("%s: inode/dev changed between lstat and open (was %d/%d, now %d/%d); directory entry was raced", soPath, lStat.Dev, lStat.Ino, fStat.Dev, fStat.Ino)
		}
	}
...
func ensurePluginCacheDir() (string, error) {
	...
	if runtime.GOOS != "windows" {
		...
		if stat.Uid != uint32(uid) {
			return "", fmt.Errorf("cache dir %s owner uid=%d does not match gateway uid=%d (someone else owns the cache dir; refuse to use)", dir, stat.Uid, uid)
		}
	}
	return dir, nil
}
