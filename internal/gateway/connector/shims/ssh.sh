...
RESPONSE=$("$CURL_BIN" -s -w "\n%{http_code}" -X POST "http://${API_ADDR}/api/v1/inspect/tool" \
  ...
  -H "X-DefenseClaw-Client: shim/ssh/1.0" \
...
if [ -z "$HTTP_CODE" ] || [ "$HTTP_CODE" -lt 200 ] 2>/dev/null || [ "$HTTP_CODE" -ge 300 ] 2>/dev/null; then
  echo "DefenseClaw: ssh shim refusing to exec real binary (gateway returned HTTP ${HTTP_CODE:-unknown})" >&2
  exit 1
fi

ACTION=$(echo "$RESULT" | jq -r '.action // "allow"' 2>/dev/null) || {
  echo "DefenseClaw: ssh shim refusing to exec real binary (malformed inspect response)" >&2
  exit 1
}
