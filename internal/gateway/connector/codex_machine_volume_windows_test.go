...
func TestValidateWindowsCodexMachineVolumeRejectsRawPerLogonAlias(t *testing.T) {
	target := t.TempDir()
	...
	if mask, err := windows.GetLogicalDrives(); err != nil ||
		mask&(uint32(1)<<('N'-'A')) != 0 {
		t.Skip("N: is unavailable for raw-alias regression")
	}
	...
	if err := validateWindowsCodexMachineVolume(
		filepath.Join(aliasRoot, "requirements.toml"),
	); err == nil || !strings.Contains(strings.ToLower(err.Error()), "mount-manager") {
