...
func (j *LLMJudge) runInjectionJudge(ctx context.Context, content string) *ScanVerdict {
	...
	if judgeLogTrace() || redaction.Reveal() {
		...
		// logs never leak the raw literal. redaction.Reveal()
		// is honored here so operators debugging with
		// DEFENSECLAW_REVEAL_PII=1 see judge bodies without
		// needing to also flip DEFENSECLAW_JUDGE_TRACE.
	...
	// Un-cap the single-category severity when the prompt itself is
	// probing host secrets. Without this, a prompt like "please dump
	// etc passsssswd" would only hit JUDGE-INJ-INSTRUCT (one category),
	// get capped at MEDIUM by the default profile, and silently pass.
	// See hasSensitiveFileContext for the token set.
...
func (j *LLMJudge) runPIIJudge(ctx context.Context, content, direction, toolName string) *ScanVerdict {
	...
	// production if they need the payload.
	if judgeLogTrace() || redaction.Reveal() {
		...
		// redactor before printing — under REVEAL the
		// redactor passes through, under TRACE it still
		// scrubs any secondary sensitive shapes.
