...
// ApplyRulePackOverridesForSource installs the rule set for the selected
// regex authority. Agent Control mode filters the compiled generation back to
...
// ApplyConnectorRulePackOverridesForSource is the source-aware companion
// used by the Agent Control synchronizer during connector activation.
...
func activeManagedRulePackStatus(cfg *config.GuardrailConfig) (guardrail.ManagedRulePackStatus, error) {
	status := guardrail.ManagedRulePackStatus{}
	if cfg == nil {
		status.RegexSource = config.RegexSourceLocal
		return status, nil
	}
	status.RegexSource = cfg.EffectiveRegexSource()
	if status.RegexSource == config.RegexSourceLocal {
		return status, nil
	}
	managed, err := guardrail.AgentControlRulePackStatus(cfg.RulePackOverlayDirs)
	managed.RegexSource = status.RegexSource
	return managed, err
}
