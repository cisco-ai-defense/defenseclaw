...
func TestOpenCodeChildSessionParentsToParentConversationTrace(t *testing.T) {
	...
	if len(spans) != 2 {
		t.Fatalf("spans=%d want parent+child anchors", len(spans))
	}
...
func TestLifecycleOnlySessionEndProducesLogAndTrace(t *testing.T) {
	...
	if got := len(exporter.GetSpans()); got != 2 {
		t.Fatalf("spans=%d want realtime anchor+terminal transition", got)
	}
...
func TestCodexHookCompletedTurnEmitsCanonicalGenAISpan(t *testing.T) {
	...
	spans := exporter.GetSpans()
	if len(spans) != 3 {
		t.Fatalf("spans=%d want realtime anchor+terminal transition+chat", len(spans))
	}
	span := spanByOperation(t, spans, "chat")
	agent := spanByOperation(t, spans, "invoke_agent")
	...
	api.emitCodexHookLLMEvent(context.Background(), stop, nil, nil)
	if got := len(exporter.GetSpans()); got != 3 {
		t.Fatalf("spans after duplicate Stop=%d want 3", got)
	}
}
...
func TestGenericHookExportsAtModelCompletionWithoutStop(t *testing.T) {
	...
	spans := exporter.GetSpans()
	if len(spans) != 2 {
		t.Fatalf("spans=%d want agent+chat before Stop", len(spans))
	}
	chat := spanByOperation(t, spans, "chat")
	attrs := spanAttributeStrings(chat)
...
func TestCodexHookCompletedToolExportsBeforeSessionStop(t *testing.T) {
	...
	spans = exporter.GetSpans()
	if len(spans) != 2 {
		t.Fatalf("spans=%d want session agent + completed tool before Stop", len(spans))
	}
	toolSpan := spanByOperation(t, spans, "execute_tool")
	agent := spanByOperation(t, spans, "invoke_agent")
	if agent.SpanContext.SpanID() != root.SpanContext.SpanID() {
		t.Fatalf("session root changed between hook events")
	}
	if agent.Parent.IsValid() {
		t.Fatalf("hook agent retained filtered HTTP parent %s", agent.Parent.SpanID())
	}
	...
	// A later tool remains a sibling in the same trace and must not create a
	// second session root.
	pre2 := pre
	...
	spans = exporter.GetSpans()
	if len(spans) != 3 {
		t.Fatalf("spans after second tool=%d want one root + two tools", len(spans))
	}
	toolCount := 0
	for _, candidate := range spans {
		...
		toolCount++
		if candidate.Parent.SpanID() != root.SpanContext.SpanID() ||
			candidate.SpanContext.TraceID() != root.SpanContext.TraceID() {
			t.Fatalf("tool %q is not attached to the session root", candidate.Name)
		}
	}
	if toolCount != 2 {
