...
// migrateToolChainTwentyFourSlotS4UState widens the fixed digest encodings for
// slot twenty-four and admits the enforcement-capable S4U ticket-to-secretsdump
// chain to the durable receipt catalog. Process-keyed pending actions, event
// windows, and partitions are ephemeral and intentionally reset. Durable deny
// receipts and replay cutoffs are preserved.
func migrateToolChainTwentyFourSlotS4UState(ex dbExecer) error {
	if _, err := ex.Exec(`ALTER TABLE guardrail_chain_deny_receipts
		RENAME TO guardrail_chain_deny_receipts_pre_twenty_four`); err != nil {
		return fmt.Errorf("stage pre-twenty-four-slot deny receipts: %w", err)
	}
	if _, err := ex.Exec(`
	...
	`); err != nil {
		return fmt.Errorf("reset pre-twenty-four-slot guardrail chain state: %w", err)
	}
	if err := migrateToolChainState(ex); err != nil {
		return fmt.Errorf("rebuild twenty-four-slot guardrail chain state: %w", err)
	}
	if err := migrateToolChainPendingState(ex); err != nil {
		return fmt.Errorf("rebuild twenty-four-slot pending chain state: %w", err)
	}
	...
	if _, err := ex.Exec(`
		INSERT INTO guardrail_chain_deny_receipts (
			...
			first_observed_time_unix_nano, last_observed_time_unix_nano,
			expires_time_unix_nano, evaluation_id, audit_event_id
		FROM guardrail_chain_deny_receipts_pre_twenty_four;
		DROP TABLE guardrail_chain_deny_receipts_pre_twenty_four;
	`); err != nil {
		return fmt.Errorf("restore pre-twenty-four-slot deny receipts: %w", err)
	}
	...
	if err := migrateToolChainState(ex); err != nil {
		return fmt.Errorf("index twenty-four-slot guardrail chain state: %w", err)
	}
	return nil
}
