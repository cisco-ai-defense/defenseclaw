...
func TestToolChainTwentyThirdSlotRoundTripsAtMaximumCapacity(t *testing.T) {
	...
	if len(encodedResources) != 1011 || len(encodedValues) != 16191 {
		t.Fatalf("maximum encoded widths=%d/%d want 1011/16191",
	...
	if decodedResources != resources || decodedValues != values {
		t.Fatal("twenty-three-slot maximum-capacity encoding did not round trip")
	}
}
