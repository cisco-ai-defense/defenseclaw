...
func (w *InstallWatcher) Run(ctx context.Context) error {
	...
	// hardening (S2.watcher, "Post-admission file changes
	...
	for {
		...
		case event, ok := <-fsw.Events:
			...
			// hardening (S2.watcher): accept Write/Remove
...
func (w *InstallWatcher) runAdmission(ctx context.Context, evt InstallEvent) (res AdmissionResult) {
	...
	defer func() {
		...
		// hardening (S2.watcher, "Post-admission file
	...
	// an explicit operator allow that recorded a
	...
	if existing, _ := pe.GetAction(targetType, evt.Name); existing != nil {
		if existing.Actions.Install == "allow" && existing.SourcePath != "" && existing.SourcePath != evt.Path {
			_ = w.logger.LogAction("install-allow-path-mismatch", evt.Path,
				fmt.Sprintf("type=%s name=%s allowed_path=%q presented_path=%q",
			...
			if w.otel != nil {
				w.otel.EmitPolicyDecision("admission", "allow-path-mismatch", evt.Name, targetType,
					"allow entry pinned to different source_path; failing closed",
					map[string]string{
			...
			res = AdmissionResult{
				...
				Reason:  "allow entry pinned to different source_path; failing closed",
			}
			return res
		}
	}
	...
	if err != nil {
		...
		// a scanner error must NOT leave the
		...
		_ = w.logger.LogAction("install-blocked", evt.Path,
			fmt.Sprintf("type=%s reason=scanner-error scanner=%s",
				targetType, s.Name()))
		w.recordAdmission(ctx, "scan-error", targetType)
	...
	if w.opa != nil {
		// hardening (S2.scanners): propagate the scanner
...
//
// hardening (S2.watcher, "Post-admission file changes inside
