...
func TestExactPOSIXSystemLogDestruction(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "sudo multi log truncate", command: `sudo truncate -s 0 /var/log/auth.log /var/log/syslog 2>/dev/null`, want: true},
		{name: "ignore exact truncate failure", command: `sudo truncate -s 0 /var/log/auth.log 2>/dev/null || true`, want: true},
		{name: "ignore nonexact truncate failure", command: `sudo truncate -s 0 /var/log/auth.log 2>/dev/null || echo failed`},
		{name: "auth log exact descendant", command: `/usr/bin/truncate -s 0 /var/log/auth.log/current.log 2>/dev/null || true`, want: true},
		{name: "auth log arbitrary descendant", command: `truncate -s 0 /var/log/auth.log/archive.log`},
		{name: "unlock and truncate same rotated audit log", command: "chattr -i /var/log/audit/audit.log.1 2>/dev/null;\ntruncate -s 0 /var/log/audit/audit.log.1", want: true},
		{name: "unlock and truncate different rotated audit logs", command: `chattr -i /var/log/audit/audit.log.1; truncate -s 0 /var/log/audit/audit.log.2`},
		{name: "exact shell wrapper", command: `/bin/sh -c 'truncate -s 0 /var/log/auth.log 2>/dev/null'`, want: true},
		{name: "journal wildcard delete", command: `sudo rm -f /var/log/journal/*/system.journal`, want: true},
		{name: "remove", command: `rm -rf /var/log/syslog`, want: true},
		{name: "remove exact security logs", command: `rm -rf /var/log/secure /var/log/wtmp`, want: true},
		{name: "remove exact security logs reversed flag", command: `rm -fr /var/log/wtmp /var/log/secure`, want: true},
		...
		{name: "multiple canonical logs", command: `rm -rf /var/log/messages /var/log/security`, want: true},
		{name: "mixed security and application logs", command: `rm -rf /var/log/secure /var/log/nginx/access.log`},
		{name: "security log backup", command: `rm -rf /var/log/secure /var/log/wtmp.1`},
		{name: "security log glob", command: `rm -rf /var/log/secure /var/log/wtmp*`},
		{name: "security log variable", command: `rm -rf /var/log/secure "$ACCOUNTING_LOG"`},
		{name: "security log documentation", command: `printf '%s\n' 'rm -rf /var/log/secure /var/log/wtmp'`},
		{name: "security log conditional", command: `test -f /var/log/wtmp && rm -rf /var/log/secure /var/log/wtmp`},
		{name: "journal retention find", command: `sudo find /var/log/journal -type f -name '*.journal' -mtime +0 -delete`},
		{name: "journal retention delete", command: `find /var/log/journal -type f -mtime +30 -delete`},
		{name: "rotated security log", command: `rm -rf /var/log/secure.1 /var/log/wtmp.1`},
		{name: "mixed truncate targets", command: `truncate -s 0 /var/log/auth.log /var/log/nginx/access.log`},
		{name: "stdout suppression", command: `truncate -s 0 /var/log/auth.log >/dev/null`},
		{name: "journal sibling wildcard", command: `rm -f /var/log/journal/*/user.journal`},
