...
func TestStaticCurlTelnetOptionRequestComponentsForFactsIsolation(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "SOCKS auth selection flags are inert without a proxy", command: "curl --socks5-gssapi --no-socks5-basic -t TTYPE=" + token + " telnet://sink.example/", want: true},
		{name: "positive SOCKS GSSAPI compatibility is capability dependent", command: "curl --socks5-gssapi-nec -t TTYPE=" + token + " telnet://sink.example/"},
		{name: "disabled SOCKS GSSAPI compatibility is build independent", command: "curl --no-socks5-gssapi-nec -t TTYPE=" + token + " telnet://sink.example/", want: true},
