...
func TestExactCloudAuditControlDestruction(t *testing.T) {
	tests := []struct {
		...
		{name: "aws delete one flow log", command: "aws ec2 delete-flow-logs --flow-log-ids fl-0e17aa62a21d4bbfe", want: true},
		{name: "aws delete one flow log with region", command: "aws ec2 delete-flow-logs --region us-east-1 --flow-log-ids fl-0e17aa62a21d4bbfe", want: true},
		{name: "aws delete resolver query log", command: "aws route53resolver delete-resolver-query-log-config --resolver-query-log-config-id rqlc-4473f20ca554c07", want: true},
		{name: "aws disable management events", command: `aws cloudtrail put-event-selectors --trail-name production-trail --event-selectors '[{"ReadWriteType":"ReadOnly","IncludeManagementEvents":false}]'`, want: true},
		{name: "aws disable management events with region", command: `aws cloudtrail put-event-selectors --event-selectors '[{"IncludeManagementEvents":false,"ReadWriteType":"ReadOnly"}]' --region us-east-1 --trail-name production-trail`, want: true},
		...
		{name: "aws flow log missing id", command: "aws ec2 delete-flow-logs"},
		{name: "aws flow log multiple ids", command: "aws ec2 delete-flow-logs --flow-log-ids fl-0e17aa62a21d4bbfe fl-1e17aa62a21d4bbff"},
		{name: "aws flow log malformed id", command: "aws ec2 delete-flow-logs --flow-log-ids fl-production"},
		{name: "aws flow log dynamic id", command: `aws ec2 delete-flow-logs --flow-log-ids "$FLOW_LOG_ID"`},
		{name: "aws resolver malformed id", command: "aws route53resolver delete-resolver-query-log-config --resolver-query-log-config-id rqlc-production"},
		{name: "aws selector keeps management events", command: `aws cloudtrail put-event-selectors --trail-name production-trail --event-selectors '[{"ReadWriteType":"All","IncludeManagementEvents":true}]'`},
		{name: "aws selector read only but included", command: `aws cloudtrail put-event-selectors --trail-name production-trail --event-selectors '[{"ReadWriteType":"ReadOnly","IncludeManagementEvents":true}]'`},
		{name: "aws selector missing trail", command: `aws cloudtrail put-event-selectors --event-selectors '[{"ReadWriteType":"ReadOnly","IncludeManagementEvents":false}]'`},
		{name: "aws selector extra field", command: `aws cloudtrail put-event-selectors --trail-name production-trail --event-selectors '[{"ReadWriteType":"ReadOnly","IncludeManagementEvents":false,"DataResources":[]}]'`},
		{name: "aws selector dynamic document", command: `aws cloudtrail put-event-selectors --trail-name production-trail --event-selectors "$SELECTORS"`},
