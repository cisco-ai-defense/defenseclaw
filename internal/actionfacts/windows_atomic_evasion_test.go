...
func TestExactWindowsTelemetryDisable(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "disable EventLog service startup", command: `REG ADD HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\EventLog /t REG_DWORD /v Start /d 4`, dialect: DialectCMD, want: true},
		{name: "restore EventLog service startup", command: `REG ADD HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\EventLog /t REG_DWORD /v Start /d 2`, dialect: DialectCMD},
		{name: "disable nearby service startup", command: `REG ADD HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Example /t REG_DWORD /v Start /d 4`, dialect: DialectCMD},
