...
import { HEADER_DEFENSECLAW_AGENT_ID, HEADER_DEFENSECLAW_AGENT_INSTANCE_ID, HEADER_DEFENSECLAW_AGENT_NAME, HEADER_DEFENSECLAW_POLICY_ID, HEADER_DEFENSECLAW_RUN_ID, HEADER_DEFENSECLAW_SESSION_ID, HEADER_DEFENSECLAW_SIDECAR_INSTANCE_ID, HEADER_DEFENSECLAW_TRACE_ID, } from "./correlation-headers.js";
...
export function applyProviderRegistry(reg) {
    ...
    for (const p of reg.providers ?? []) {
        ...
        for (const d of p?.domains ?? []) {
            ...
            if (norm === "" || /[\s/\\]/.test(norm) || norm.includes("://"))
                continue;
    ...
    for (const port of reg.ollama_ports ?? []) {
        if (portsAdded >= MAX_PORT_ADDITIONS)
            break;
 ...
 */
export async function bootstrapProviderOverlay(guardrailPort, options) {
    ...
    try {
        ...
        if (resBody && typeof resBody.getReader === "function") {
            ...
            if (t.length > MAX_RESPONSE_BYTES)
                return;
            text = t;
        }
        else {
...
export function isLLMUrl(url, guardrailPort) {
    const host = extractHost(url);
    if (matchesLLMDomain(host))
        return true;
...
export const LLM_PATH_SUFFIXES = [
    "/chat/completions",
    "/completions",
    "/messages",
    ":generateContent",
...
export function classifyBodyShape(body) {
 ...
 */
export function isLLMShapedRequest(urlStr, method, bodyShape, guardrailPort) {
    if (!urlStr)
        return false;
 ...
 */
function extractProviderKey(headers) {
 ...
 * case-sensitive keys).
 */
function extractProviderKeyFromRecord(hdrs) {
 ...
 */
function buildProxyHeaders(targetOrigin, providerKey, getCorrelationHeaders) {
    const hdrs = {
 ...
 */
export function createFetchInterceptor(portOrOpts) {
    const interceptorOpts = typeof portOrOpts === "number"
        ? { guardrailPort: portOrOpts }
        : portOrOpts;
    const guardrailPort = interceptorOpts.guardrailPort;
    const getCorrelationHeaders = interceptorOpts.getCorrelationHeaders ?? emptyCorrelationHeaders;
    ...
    function start() {
        ...
        function patchedHttpsRequest(urlOrOptions, optionsOrCallback, callback) {
            ...
            if (urlStr && (knownForHTTPS || shapedForHTTPS)) {
                ...
                const proxyHdrs = buildProxyHeaders(originalUrl.origin, providerKey, getCorrelationHeaders);
                ...
                const { host: _legacyHost, agent: _legacyAgent, ca: _ca, cert: _cert, key: _key, pfx: _pfx, passphrase: _passphrase, servername: _servername, ciphers: _ciphers, ecdhCurve: _ecdhCurve, secureProtocol: _secureProtocol, minVersion: _minVersion, maxVersion: _maxVersion, sigalgs: _sigalgs, crl: _crl, dhparam: _dhparam, rejectUnauthorized: _rejectUnauthorized, checkServerIdentity: _checkServerIdentity, session: _session, allowPartialTrustChain: _allowPartialTrustChain, ...restOpts } = opts;
                void _legacyHost;
                void _legacyAgent;
                void _ca;
                void _cert;
