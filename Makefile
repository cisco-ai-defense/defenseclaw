...
dist: dist-cli dist-gateway dist-plugin dist-sandbox dist-upgrade-manifest dist-checksums
	...
	@echo ""
	@echo "Cut a release (certify first, then promote the exact certified bytes):"
	@echo "  gh workflow run release.yaml --ref main -f operation=certify -f version=X.Y.Z"
	@echo "  # Wait for certification to succeed, then:"
	@echo "  gh workflow run release.yaml --ref main -f operation=release -f version=X.Y.Z -f immutable_releases_confirmed=true"
	@echo ""
